            ...
            async for self in tasks:
                ...
                if self.last_end_time is None and self.last_start_time is not None:
                    ...
                    if run_duration > timedelta(hours=2):
                        msg = "Kill {} because it has been running more than 2 hours".format(astr(self))
                        await ar.adebug(msg)
                ...
                nst = await sync_to_async(self.get_next_suggested_date)(self.last_end_time, ar.logger)
                next_time = min(next_time, nst)
...
async def start_log_server():
    ...
    if log_sock_path is None:
        logger.info("No log server because there is no directory %s.",
            site.site_dir / 'log')
        return
