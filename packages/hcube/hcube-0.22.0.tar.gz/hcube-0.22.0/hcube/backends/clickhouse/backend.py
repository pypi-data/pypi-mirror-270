...
class ClickhouseCubeBackend(CubeBackend):
        ...
        if joins:
            join_part = " ".join(
                [
                    ...
                    f"{join.dimension.name} = {join.joined_dim.name}"
                    for join in joins
                ]
            )

        # put it together
        text = f"SELECT {select} FROM {table} {join_part} {final_part} "
        if where:
            text += f"WHERE {where} "
        if group_by:
    ...
    def _create_materialized_view(
        ...
        mv_command = (
            f"CREATE MATERIALIZED VIEW IF NOT EXISTS {backing_table_name}_MV "
