...
class AggregatingMaterializedViewMeta(type):
    ...
    def _process_attrs(cls):
        ...
        if excludes_dims:
            ...
            meta = cls.cube.Clickhouse if hasattr(cls.cube, "Clickhouse") else None
            sorting_key = getattr(meta, "sorting_key", []) if meta else []
            # add dimensions from sorting key
            for dim_name in sorting_key:
                if dim_name not in excludes_dims:
                    cls._dimensions[dim_name] = cls.cube._dimensions[dim_name]
