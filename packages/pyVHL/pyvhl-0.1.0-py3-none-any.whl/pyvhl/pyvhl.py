...
class pyvhl:
    ...
    def __init__(self) -> None:
        session = Session()
        session.headers["User-Agent"] = "mirror_bot"
        self.client = Client(session=session)
        self.clients = {
            "juststream": self.client.juststreamlive,
            "streamable": self.client.streamable,
            "streamja": self.client.streamja,
            "streamff": self.client.streamff,
            # "streamwo": self.client.streamwo,
            # "mixture": self.client.mixture,
        }
    ...
    def get_video(self, video_url: str, download: bool = True) -> dict:
        ...
        if info_dict["extractor"] == "twitch:clips":
            ...
            logger.error(
                f"Clip URL: {video_url} | Clip not available"
            )
            return {"title": None, "url": None, "id": None, "streamer": None, "date": None, "file_size": None}

    @retry(tries=10, delay=5)
    def upload_video(self, clip_title: str, id: int):
        ...
        Returns:
            str: Mirror url
        """
        client_name = random.choice(list(self.clients))
...
if __name__ == "__main__":
    pyvhl = pyvhl()
    result = pyvhl.get_video(
        "https://clips.twitch.tv/LongAdventurousSandpiperMrDestructoid-Ug1mcxeNeiGvVbW9"
    )
    print(result)
    mirror_url = pyvhl.upload_video(result["title"], result["id"])
    print(mirror_url)
