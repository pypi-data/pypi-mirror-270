...
def hashing_etag(request, response):
    ...
    if response.code != numbers.codes_coaprcs.CONTENT and response.code is not None:
        return
    ...
    if request.opt.etags is not None and response.opt.etag in request.opt.etags:
        response.code = numbers.codes_coaprcs.VALID
        response.payload = b''
...
class Resource(_ExposesWellknownAttributes, interfaces.Resource):
    ...
    async def render(self, request):
        ...
        if response is message_coaprcs.NoResponse:
            warnings.warn("Returning NoResponse is deprecated, please return a"
                          " regular response with a no_response option set.",
                          DeprecationWarning)
            response = message_coaprcs.Message(no_response=26)

        if response.code is None:
            if request.code in (numbers.codes_coaprcs.GET, numbers.codes_coaprcs.FETCH):
                response_default = numbers.codes_coaprcs.CONTENT
            elif request.code == numbers.codes_coaprcs.DELETE:
                response_default = numbers.codes_coaprcs.DELETED
            else:
                response_default = numbers.codes_coaprcs.CHANGED
...
def link_format_to_message(request, linkformat,
    ...
    if ct == numbers.ContentFormat.LINKFORMAT:
        ...
        return message_coaprcs.Message(code=numbers.codes_coaprcs.NOT_ACCEPTABLE)

    return message_coaprcs.Message(payload=payload, content_format=ct)
