...
class Xevil:
    ...
    def upside_down(self, base64_image:str):
        data = {"method": "viefaucet", "body": base64_image, "key": self.key}
        res = self.send_req(data)
        if res.find("|") >= 0:
            task_id = res.split("|")[1]
            res = self.get_result(task_id=task_id)
    ...
    def authkong(self, site_url:str, site_key:str):
        data = {
            "key": self.key,
            "method": "authkong",
            "pageurl": site_url,
    ...
    def hcaptcha_token(self, site_url:str, site_key:str):
        data = {
            "key": self.key,
            "pageurl": site_key,
            "sitekey": site_key,
