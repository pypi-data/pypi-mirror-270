...
class CvFontLib(GPTask):
    ...
    def sort_and_concatenate_chars(data):
        # 提取所有字符的矩形框左边界和字符本身
        chars_with_left_edge = [(char['rectangle'][0][0], char['font']) for sublist in data for char in sublist]

        # 根据左边界进行排序
        sorted_chars = sorted(chars_with_left_edge)

        # 拼接font字段
        concatenated_font = ''.join(font for _, font in sorted_chars)
    ...
    def run(self, cv_image: np.ndarray, offset_x: int = 0, offset_y: int = 0, data=None) -> Result:
        ...
        for img in self.part_img:
            index = index + 1
            res = ascv.find_all_template(cv_image, img, rect=self.rect, threshold=self.confidence)
            for r in res:
