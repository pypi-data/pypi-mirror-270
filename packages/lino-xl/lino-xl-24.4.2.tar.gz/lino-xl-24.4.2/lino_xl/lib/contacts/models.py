...
def show_birthdays(ar, today=None):
    ...
    def fmt(obj, day=True):
        if day:
            s =  str(obj.birth_date)[-5:] + " "
        else:
            s = ""
        s += ar.obj2htmls(obj, obj.get_full_name(salutation=False))
        if obj.birth_date.year:
            s += " ({})".format(today.year - obj.birth_date.year)
        return s

    if todayqs.count():
        yield ("<p><b>{}</b>: ".format(_("Birthdays today")) +
               ", ".join([fmt(obj, day=False) for obj in todayqs]) + "</p>")
    if recent.count():
