...
class Spectrum:
    ...
    def only_positive_frequencies(self):
        positive_freq_indices = np.where(self.frequencies > 0)
        self.frequencies = self.frequencies[positive_freq_indices]
        self.amplitudes = self.amplitudes[positive_freq_indices]
        if self.spl != None:
            self.spl = self.spl[positive_freq_indices]

    def cutoff_frequency(self, lower_freq = -1e-9, higher_freq = 1e+9):
        condition = np.logical_and(lower_freq<self.frequencies, higher_freq>self.frequencies)
        indices = np.where(condition)
        self.frequencies = self.frequencies[indices]
        self.amplitudes = self.amplitudes[indices]
        if self.spl != None:
            self.spl = self.spl[indices]
