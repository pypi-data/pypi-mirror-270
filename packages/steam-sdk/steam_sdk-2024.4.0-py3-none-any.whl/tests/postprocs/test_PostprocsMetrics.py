...
import steam_sdk.postprocs.PostprocsMetrics
...
class TestPostprocsMetrics(unittest.TestCase):
    ...
    def test_RELATIVE_RMSE_AFTER_t_PC_off(self):
        ...
        #arrange
        var_of_interest_overlap = [3.047619047619044, 3.6238095238095234, 4.2, 5.566666666666667, 8.4, 7.928571428571427, 2.5]
        time_stamps_overlap = [0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0]
        var_ref_overlap = [0.0, 0.8125, 2.6500000000000004, 5.5, 3.0000000000000004, 10.0, 2.0]

        metrics_to_do = ['RELATIVE_RMSE_AFTER_t_PC_off']

        Target_RMSE = 0.27593789122083673


        #act
        metrics = PostprocsMetrics(metrics_to_do=metrics_to_do, var_to_interpolate=var_of_interest_overlap,
                                   var_to_interpolate_ref=var_ref_overlap, time_vector=time_stamps_overlap,
                                   time_vector_ref=time_stamps_overlap)

        #assert
        self.assertEqual(metrics.metrics_result[0], Target_RMSE)
