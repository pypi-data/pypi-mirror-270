    ...
    def drop_temp_tables(self):
        ...
        for tbl in tables:
            if tbl["table_type"] == "VIEW":
                self.execute("DROP VIEW IF EXISTS %s.%s CASCADE;" % (tbl["table_schema"], tbl["table_name"]))
            else:
                self.execute("DROP TABLE IF EXISTS %s.%s CASCADE;" % (tbl["table_schema"], tbl["table_name"]))
