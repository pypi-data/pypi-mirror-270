...
def create_cast_rule(rule, name=None, cost=1):
    ...
    """
    return CastLambda(rule=rule, name=name, cost=cost)


def create_conversion_rule(rule):
    return ConversionLambda(rule=rule)
...
CV_RULEBOOK = (AutoRuleBook(id="CV_RULEBOOK").add_rules(
    ...
    create_alias_rule("numpy_bgr", "numpy,uint8,HWC,BGR,0_255"),
    create_alias_rule("numpy_hw1", "numpy,uint8,HWC,L,0_255"),
    create_alias_rule("numpy_hw", "numpy,uint8,HW,L,0_255"),
