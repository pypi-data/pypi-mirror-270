...
class MolecularDynamic_builder(PartitionManager, ):
    ...
    def plot_path(self, positions, bins:int=100, output_path:str=None, transparency:bool=True, save:bool=True, verbose:bool=True):
        """
        """    
        d_name = {0:'x', 1:'y', 2:'z', }
        for d in range(3):
            for u in self.containers[0].AtomPositionManager.uniqueAtomLabels:
                ...
                ax1.plot(positions[:, mask, d], 'o', ms=1, alpha=0.2, lw=0.7, color=color)
                ...
                for d2 in range(d+1,3):
                    ...
                    ax1.plot(positions[:, mask, d], positions[:, mask, d2], 'o', ms=1, color=color, alpha=0.2, lw=0.8)
    ...
    def handleMDAnalysis(self, values:list ):
        ...
        for plot in values:
            ...
            if plot.upper() == 'PATH_TRACKING':
                ...
                self.plot_path( positions=MDA_data['positions'], output_path=values[plot].get('output_path', '.'), transparency=values[plot].get('transparency', False), verbose=values[plot].get('verbose', False) )
