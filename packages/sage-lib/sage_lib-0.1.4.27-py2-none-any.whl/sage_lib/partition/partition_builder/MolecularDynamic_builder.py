...
class MolecularDynamic_builder(PartitionManager, ):
    ...
    def get_positions(self, wrap:bool=True):
        """
        """
        if wrap:
    ...
    def plot_path(self, positions, bins:int=100, output_path:str=None, save:bool=True, verbose:bool=True):
        """
        """    
        d_name = {0:'x', 1:'y', 2:'z', }
        for d in range(3):
            for u in self.containers[0].AtomPositionManager.uniqueAtomLabels:
                ...
                ax1.plot(positions[:, mask, d], alpha=0.2, lw=0.9)
                ...
                plt.tight_layout()
                if save:
                    plt.savefig(f'{output_path}/PATH_TRACKING_{u}_{d_name[d]}.png', dpi=300, transparent=True)
                ...
                for d2 in range(d+1,3):
                    ...
                    ax1.plot(positions[:, mask, d], positions[:, mask, d2], color=color, alpha=0.2, lw=0.9)
                    ...
                    plt.tight_layout()
                    if save:
                        plt.savefig(f'{output_path}/PATH_TRACKING_{u}_{d_name[d]}-{d_name[d2]}.png', dpi=300, transparent=True)
    ...
    def handleMDAnalysis(self, values:list ):
        ...
        for plot in values:
            ...
            if plot.upper() == 'PATH_TRACKING':
                MDA_data['positions'] = self.get_positions( values[plot].get('wrap', False) )
                self.plot_path( positions=MDA_data['positions'], output_path=values[plot].get('output_path', '.'), verbose=values[plot].get('verbose', False) )
