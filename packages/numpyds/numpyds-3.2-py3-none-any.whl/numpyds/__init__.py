...
columns_to_drop = ["sibsp", "parch", "embarked", "class", "who", "deck", "embark_town"]
df.drop(columns_to_drop, axis=1, inplace=True)
df

# Checking and replacing/dropping NULL values
...
print("Correlation of Fare with Other Feature")
corr = df.corr(method="pearson", numeric_only=True)
print(corr)

x = df["fare"]
y = df["pclass"]
deviation_x = x - np.mean(x)
deviation_y = y - np.mean(y)
sum_deviation_product   = np.sum(deviation_x*deviation_y)
sum_deviation_x_squared = np.sum(deviation_x**2)
sum_deviation_y_squared = np.sum(deviation_y**2)
total = np.sqrt(sum_deviation_x_squared*sum_deviation_y_squared)
correlation = sum_deviation_product/total
print("\nCorrelation between Fare and Pclass type is:", correlation)
...
print("Covariance between Fare and Pclass is:", covariance)
...
def calculateBinomCoef(n, k, p):
    q = 1 - p
    pOfX = math.comb(n, k) * (p ** k) * (q ** (n - k))
    return pOfX
...
def calculatePoissionDist(mu, x):
    factOfX = calculateFact(x)
    poissionOfX = ((mu ** x) * math.exp(-mu)) / factOfX
    return poissionOfX

# Uniform distribution
def calculateUniformProbabilities():
    probability = []
    for x in range(30, 36):
        prob = (x - 27) / 12
        probability.append(prob)
    return probability, probability[-1] - probability[0]

def calculateVarianceAndStdDev(numbers):
    mean = np.mean(numbers)
    sumofximinusmui = sum((x - mean) ** 2 for x in numbers)
    variance = sumofximinusmui / len(numbers)
    standard_deviation = math.sqrt(variance)
    return variance, standard_deviation

# Normal distribution
def calculateNormalDist(x, mu, sigma):
    pi = math.pi
    normalDist = ((1 / (sigma * math.sqrt(2 * pi))) * math.exp((-1 / 2) * (((x - mu) / sigma) ** 2)))
    return normalDist

# Examples
# 1) Binomial
print("Probability for exactly 19 very satisfied:", calculateBinomCoef(25, 19, 0.65))

# 2) Poisson
print("Poisson probability for mu=2, x=3:", calculatePoissionDist(2, 3))

# 3) Uniform
probability, prob_range = calculateUniformProbabilities()
print("Probabilities:", probability)
print("Probability between 30 to 35:", prob_range)

# 4) Variance and Standard Deviation for Uniform Distribution
insurance_costs = np.arange(200, 1183)
variance, std_deviation = calculateVarianceAndStdDev(insurance_costs)
print("Variance:", variance)
print("Standard Deviation:", std_deviation)

# 5) Normal distribution
print("Normal distribution:", calculateNormalDist(100, 494, 700))
