...
<head>
    ...
    <style>
        ...
        *::after {
        ...
        .address-minter-input input[type="text"] {
            border-radius: 8px;
            text-align: center;
            width: 100%;
            height: 40px;
        ...
        .address-minter-input,
        .address-collection-input {
            width: 50%;
        }
...
<body>
    <div class="container">
        <div class="header">
            <div class="subscribe-text"><a id="link" href="https://t.me/ferey56666666666_bot">@ferey56666666666_bot</a>
            </div>
            <div id="connect"></div>
        </div>
        <div class="subscribe">
            <div class="subscribe-inner">
                <div class="subscribe-text">Подписка:</div>
        ...
        <div class="address-minter">
            <div class="address-minter-text">Адрес коллекции монет:*</div>
            <div class="address-minter-input"><input type="text" id="address-minter-input-id" minlength="48"
                    maxlength="48" placeholder="для других валют">
            </div>
        </div>
        <div class="address-collection">
            <div class="address-collection-text">Адрес коллекции нфт:*</div>
            <div class="address-collection-input"><input type="text" id="address-collection-input-id" minlength="48"
                    maxlength="48" placeholder="для проверки нфт">
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {
            tg.MainButton.show().setParams({ text: "Сохранить", is_active: true, is_visible: true })
            document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-minter-input-id')).getPropertyValue('width')
            await sleep(1111)
            document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-minter-input-id')).getPropertyValue('width')
        })
        ...
        tonConnectUI.onStatusChange(async (wallet) => {
            ...
            if (tonConnectUI.connected) {
                addressOwnerRaw = tonConnectUI.account.address;
            }
            document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-minter-input-id')).getPropertyValue('width')
        })
        ...
        tg.MainButton.onClick(async () => {
            ...
            let value = document.getElementById("invoice-input").value
            let currency = document.getElementById("subscribe-datalist-id").value
            let period = document.getElementById("period").value
            let addressMinter = document.getElementById("address-minter-input-id").value
            let addressCollection = document.getElementById("address-collection-input-id").value
            console.log(value, currency, period, addressMinter, addressCollection, addressOwnerRaw)

            if (addressOwnerRaw === "") {
                tg.showAlert("Для сохранения информации необходимо подключить TON-кошелек")
                return
            }

            if (value < 0.05) {
                tg.showAlert("0.05 - minimum")
                document.getElementById("invoice-input").value = 0.05
                return
            }

            if (currency !== "TON" && addressMinter === "") {
                tg.showAlert("При указании других валют необходимо заполнить адрес коллекии монет")
                return
            }

            console.log('fetch data to back: console.log(value, currency, period, addressMinter, addressCollection, addressOwnerRaw)')
            console.log(value, currency, period, addressMinter, addressCollection, addressOwnerRaw)
            tg.close()
        })
...
<body>
    <div class="container">
        ...
        <div class="subscribe">
            <div class="subscribe-inner">
                <div class="subscribe-text">Подписка: 1 TON (месячная)</div>
            </div>
        </div>

        <div class="address-collection">
            <div class="subscribe-text">Проверка наличия NFT</div>
            <div class="address-collection-input"><input type="text" id="address-collection-input-id" minlength="48"
                    maxlength="48" placeholder="для проверки нфт">
            </div>
        </div>

        <div id="send-coin">Отправить 1.2 TON (неделя)</div>
        <div class="optional">нет активных подписок</div>
        <a id="link-coin" href="https://tonviewer.com/EQDIePxrqM9RRikDVLR6SMomjjz3apnp0rzWtdQMJDQNm2p5">Подписка
            активирована до 12/01/24</a>


        <div id="send-nft">Приобрести NFT</div>
        <div class="optional">нет необходимого NFT</div>
        <div class="optional"></div>
        <a id="link-nft" href="https://tonviewer.com/EQDIePxrqM9RRikDVLR6SMomjjz3apnp0rzWtdQMJDQNm2p5">NFT уже
            получен</a>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {
            tg.MainButton.show().setParams({ text: "Открыть группу", is_active: true, is_visible: true })
            document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-minter-input-id')).getPropertyValue('width')
            await sleep(1111)
            document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-minter-input-id')).getPropertyValue('width')
        })
        ...
        tg.MainButton.onClick(async () => {
            console.log('Кнопка была нажата');
            tg.openTelegramLink('https://t.me/hdhdjrjejrejrirjrnebe');
