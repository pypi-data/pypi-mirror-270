...
class IESLabTypicalDayResult(Result):
    def __init__(self, *args, **kwargs):
        ...
        self.__type_list =['电负荷', '热负荷','冷负荷','氢负荷','总辐射','散射辐射', '直射辐射','天顶角', '环境温度', '湿球温度','土壤温度', '10m风速', '50m风速']
    ...
    def __readPlotResult(self):
        ...
        if (length > self.__plotIndex):
            for num in range(self.__plotIndex, length):# update TypicalMonth
                ...
                if val['type'] == 'plot':
                    key_list = re.split('-month',val['key'])#格式为：散射辐射-month1，re后分别为类型和月份
                    typical_month_m = self.result['TypicalMonth'][int(key_list[1])-1]
                    typical_month_m['月份'] = int(key_list[1])
                    ...
                    for i in range(typicalNum):
                        typical_month_m[key_list[0]].append([])
                        if key_list[0]  == '环境温度':#各类版本气象数据均有环境温度数据
                    ...
                    for i in range(len(val_data_traces)):
                        typical_month_m[key_list[0]][int(re.findall('\d+',val_data_traces[i]['name'])[0])-1] = copy.deepcopy(val_data_traces[i]['y'])            
    ...
    def GetTypicalDayCurve(self,dayID, dataType):
        '''
            ...
            :params: dayID int类型，表示典型日的ID，数值位于 0~典型日数量 之间
            :params: dataType enum类型，标识辐照强度、环境温度、湿球温度、土壤温度、建筑物高度风速、风机高度风速、电负荷、热负荷、冷负荷的参数类型
