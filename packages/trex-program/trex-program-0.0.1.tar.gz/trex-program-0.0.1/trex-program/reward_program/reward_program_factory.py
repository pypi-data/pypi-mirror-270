...
class RewardProgramFactory:
    ...
    def __get_giveaway_mechanism_reward_program(self, program_configuration, currency=None):
        ...
        if reward_base == program_conf.REWARD_BASE_ON_GIVEAWAY or reward_base == program_conf.REWARD_BASE_ON_BIRTHDAY:
            if reward_format == program_conf.REWARD_FORMAT_POINT:
                return PointGiveawayProgram(program_configuration, currency=currency)
            
            elif reward_format == program_conf.REWARD_FORMAT_STAMP:
                return StampGiveawayProgram(program_configuration, currency=currency)
            
            elif reward_format == program_conf.REWARD_FORMAT_PREPAID:
                return PrepaidGiveawayProgram(program_configuration, currency=currency)
            
            elif reward_format == program_conf.REWARD_FORMAT_VOUCHER:
                return VoucherProgram(program_configuration, currency=currency)
        
        elif reward_base == program_conf.REWARD_BASE_ON_TIER:
            return TierRewardProgram(program_configuration)
        
    def __construct_reward_program(self, program_configuration, currency=None):
        desc            = program_configuration.get('desc')
        reward_base     = program_configuration.get('reward_base')
        reward_format   = program_configuration.get('reward_format')
        ...
        if reward_base in (program_conf.REWARD_BASE_ON_GIVEAWAY, program_conf.REWARD_BASE_ON_BIRTHDAY):
            ...
            if reward_format == program_conf.REWARD_FORMAT_POINT:
                return PointGiveawayProgram(program_configuration)
            
            elif reward_format == program_conf.REWARD_FORMAT_STAMP:
                return StampGiveawayProgram(program_configuration)
            
            elif reward_format == program_conf.REWARD_FORMAT_PREPAID:
                return PrepaidGiveawayProgram(program_configuration)
            
            elif reward_format == program_conf.REWARD_FORMAT_VOUCHER:
                return VoucherProgram(program_configuration)    
            ...
            if reward_format == program_conf.REWARD_FORMAT_POINT:
                ...
                return PrepaidSchemeProgram(program_configuration, currency=currency)
    ...
    def get_giveaway_reward(self, customer_acct, transaction_details, program_configuration_list=None, currency=None, reward_set=1, create_upstream=True):
        ...
        if is_not_empty(program_configuration_list):
            ...
            return self.__give_reward_based_on_reward_base(customer_acct, transaction_details, program_configuration_list, 
                                                               program_conf.REWARD_BASE_ON_GIVEAWAY, currency=currency, reward_set=reward_set, create_upstream=create_upstream)
    
    def get_birthday_reward(self, customer_acct, transaction_details, program_configuration_list=None, currency=None, reward_set=1):
        ...
        if is_not_empty(program_configuration_list):
            ...
            return self.__give_reward_based_on_reward_base(customer_acct, transaction_details, program_configuration_list, 
                                                               program_conf.REWARD_BASE_ON_BIRTHDAY, currency=currency, reward_set=reward_set)
    
    def get_spending_reward(self, customer_acct, transaction_details, program_configuration_list=None, currency=None):
        ...
        if is_not_empty(program_configuration_list):
            ...
            return self.__give_reward_based_on_reward_base(customer_acct, transaction_details, program_configuration_list, 
                                                               program_conf.REWARD_BASE_ON_SPENDING, currency=currency)
            
        return False 
            
    def get_tier_reward(self, customer_acct, transaction_details, program_configuration_list=None, currency=None):
        ...
        if is_not_empty(program_configuration_list):
            ...
            return self.__give_reward_based_on_reward_base(customer_acct, transaction_details, program_configuration_list, 
                                                               program_conf.REWARD_BASE_ON_TIER, currency=currency)          
            
    
    def get_purchase_membership_giveaway_reward(self, customer_acct, transaction_details, program_configuration_list=None, currency=None, reward_set=1):
    ...
    def __give_reward_based_on_reward_base(self, customer_acct, transaction_details, program_configuration_list, 
        ...
        for program_configuration in program_configuration_list:
            if reward_base == program_configuration.get('reward_base'):  
                ...
                reward_program = self.__construct_reward_program(program_configuration, currency=currency)
    ...
    def __give_purchase_membership_reward_by_program_configuration(self, customer_acct, transaction_details, program_configuration_list, currency=None, reward_set=1):
