...
class LogWidget(QWidget):
    def __init__(self):
        ...
        layout.addWidget(QLabel('Log widget introduces extra latency and jitter while visible.\n'
...
class LogView(QPlainTextEdit):
    ...
    def __init__(self):
        super().__init__()
        self.__anchor_text: Optional[str] = None
    ...
    def __hide_lines_not_matching_filter(self, last_n_blocks: int = 0):
        ...
        if not last_n_blocks:
            ...
            start_from = document.blockCount() - last_n_blocks
        
        for line_n in range(start_from, document.blockCount()):
            block = document.findBlockByNumber(line_n)

            if (self.__filter_words and block.text() and
                    any([filter_word.lower() not in block.text().lower()
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        if event.button() == Qt.LeftButton and self.__anchor_text:
            ...
            QToolTip().showText(event.globalPosition().toPoint(),
                                f'Copied {self.__anchor_text}', self, msecShowTime=2000)
