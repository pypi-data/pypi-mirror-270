...
class Auth(object):
  ...
  def _boto_session(self):
    if not hasattr(self, '_Auth__boto_session'):
      for k in environ.keys():
        if k.startswith('AWS_'):
          del environ[k]
      self.__boto_session = boto3.Session(region_name=self._client_region)
    return self.__boto_session

  def _auth_client(self):
    if not hasattr(self, '_Auth__auth_client'):
      self.__auth_client = self._boto_session().client('cognito-idp')
