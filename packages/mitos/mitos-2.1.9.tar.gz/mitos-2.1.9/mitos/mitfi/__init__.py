...
#             continue
        ...
        with open("%s/%s_%sout.nc" %
                  (outpath, os.path.basename(seqfile), t), "wb") as f:
            f.write(out)
...
def cmrealign(sequence, rRNA, refdir):
    ...
    # improve structure
    out, err = util.execute(["java", "-jar", mitfiimppath, tdir, "%s/cmrealign.stk" % (tdir)], InfernalException)
