...
#        ststrange = ststrange,maxdegenerate = maxdegenerate
        ...
        for f in tfl:
            ...
            f.rf = int(math.copysign(((abs(f.rf) - 1) + (offsets[i] % 3)) % 3 + 1, f.rf))
        ...
        for n in oidx:  # names
            ...
            for of in oidx[n]:  # frames
                if abs(of) == 4:
                    ...
                    f = int(math.copysign(((abs(of) - 1) + (offsets[i] % 3)) % 3 + 1, of))
...
#                 logging.warn( " ".join( [str( x.get_position() ) for x in cp[l:r + 1]] ) )
                hits.append(feature.blast_feature(
                    name, type_from_name(name), strand, cp[left:right + 1], mito=1, rf=rf))
    return hits
