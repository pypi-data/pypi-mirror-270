...
#
        ...
        if not refseq_results[i][2] == "No_OH_RefSeq" and not refseq_results[i][4] == "No_OL_RefSeq":   # both OH and OL are found in RefSeq, we only save these cases and we ignore the case where one (or both) origin is missing from RefSeq
            ...
            for xx in range(len(our_unique_arr)):
                ...
                if if_intersect(our_unique_arr[xx][0], refseq_results[i][2], refseq_results[i][3]) is True and if_intersect(our_unique_arr[xx][1], refseq_results[i][4], refseq_results[i][5]) is True:  # both are found, True
                ...
                elif if_intersect(our_unique_arr[xx][0], refseq_results[i][2], refseq_results[i][3]) is True and if_intersect(our_unique_arr[xx][1], refseq_results[i][4], refseq_results[i][5]) is False:  # found only OH, False
                    if our_unique_arr[xx][2] < 0 and our_unique_arr[xx][3] <= pval_threshold:   # correct slope and pval <= threshold -> positive
                ...
                elif if_intersect(our_unique_arr[xx][0], refseq_results[i][2], refseq_results[i][3]) is False and if_intersect(our_unique_arr[xx][1], refseq_results[i][4], refseq_results[i][5]) is True:  # found only OL, False
                    if our_unique_arr[xx][2] < 0 and our_unique_arr[xx][3] <= pval_threshold:   # correct slope and pval <= threshold -> positive
                ...
                elif if_intersect(our_unique_arr[xx][0], refseq_results[i][2], refseq_results[i][3]) is False and if_intersect(our_unique_arr[xx][1], refseq_results[i][4], refseq_results[i][5]) is False:  # found nothing, False
                    if our_unique_arr[xx][2] < 0 and our_unique_arr[xx][3] <= pval_threshold:   # correct slope and pval <= threshold -> positive
...
def _parse_feature_locations(feat):
    ...
    if len(feat.sub_features) > 0:
        ...
        loc.append((feat.location.start.position, feat.location.end.position - 1, strand))
