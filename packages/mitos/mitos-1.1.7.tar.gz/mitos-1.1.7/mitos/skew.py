...
if options.random is not None and options.random != "random" and options.random != "shuffle":
...
for arg in args:
    ...
    if options.window is None and gb is None:
                ...
                if options.window is None:
                    ...
                    for start in range(0, len(seq) - options.window, options.offset):
                        if options.Xfold is not None:
                            w = 3
                            sseq = None
                            while (sseq is None or len(sseq) < options.window) and start + w <= len(seq):
        ...
        for s in range(start, stop, options.offset):
            subs.append({
                "seq": seq.subseq(s - options.window / 2 + 1, s + options.window / 2, options.strand),
                "strand": options.strand,
                "name": "window",
    ...
    for f in subs:
        ...
        if options.Xfold is not None:
            if "fstrand" in f and f["fstrand"] != f["strand"]:
                f["seq"] = sequence(str(f["seq"].reverse_complement()))
