...
from mitos import codon
from mitos.feature import blast_feature, capcup, feature, length
from mitos.mito import types
...
#            print name, rf, mx,
        ...
        for i in list(idx[rf].keys()):
            p = idx[rf][i]
#                print p, posgenes[p]
            if ((percent and values[p].get_score(scoresel, pavg) < endcut)
                    or (not percent and values[p].get_score(scoresel, pavg) < cutoff)):
                values[p] = None
                del idx[rf][i]
...
#     print "from", frm, "rng", rng
    ...
    for i in rng:
        if not sequence.circular and (i - 2 < 0 or i + 2 >= len(sequence)):
            continue
...
#             break
    ...
    if strand == 1:
        rng = list(range(frm, posi + aarng * 3 + 1, 3))
    else:
        rng = list(range(frm, posi - aarng * 3 - 1, -3))
...
#    hits = {1:0, 2:0, 3:0, -1:0, -2:0, -3:0}
    ...
    for line in blasthandle:
        ...
        if sqn is not None and sqn == cols[1].split(":")[0]:
            continue
...
def blasthandle_xml(blastfile, minevalue, posgenes, value, query, sqn=None):
    ...
    for file in handle:
        for alignmen in file.alignments:
            ...
            # print dir(alignmen)
            if sqn is not None and sqn == alignmen.accession.split(":")[0]:
