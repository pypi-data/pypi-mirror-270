...
class InstallHandler(object):
    ...
    def on_play_start(self, play_name, host_num=1):
        self.deploy_info_list.append("Play[{}] start.".format(play_name))
        if not self.start_time:
    ...
    def on_task_start(self, task_name):
        self.deploy_info_list.append("Task[{}] start.".format(task_name))

    def on_task_end(self, task_name, status, deploy_info):
        self.deploy_info_list.append("Task[{}] {}. {}".format(task_name, status, deploy_info or ""))

    def on_task_failed(self, task_name, failed_info):
        self.deploy_status = DeployStatus.FAILED
        self.on_task_end(task_name, DeployStatus.FAILED, failed_info)

    def on_unreachable(self, host):
        self.deploy_status = DeployStatus.FAILED
        self.deploy_info_list.append("Host[{}] is unreachable.".format(host))
...
class ProgressManager(object):
    ...
    def on_task_failed(self, task_name, failed_info):
        ...
        if not self._is_handler_existed():
            return
        self.install_handlers[self.cur_handler_index].on_task_failed(task_name, failed_info)
    ...
    @error_catcher
    def on_task_end(self, task_name, deploy_status, deploy_info):
        if not self._is_handler_existed():
            return
        self.install_handlers[self.cur_handler_index].on_task_end(task_name, deploy_status, deploy_info)
...
class CallbackModule(ansible.plugins.callback.CallbackBase):
    ...
    def _build_res_info(result):
        ...
        res_info_list = ["Task on host: {}.".format(host)]
        if msg:
        ...
        return res_info_list, task_name

    def v2_runner_on_ok(self, result):
        # result: ansible.executor.task_result.TaskResult
        super(CallbackModule, self).v2_runner_on_ok(result)
        res_info_list, task_name = self._build_res_info(result)
        self.progress_manager.on_task_end(task_name, DeployStatus.SUCCESS, "\n".join(res_info_list))
    ...
    def v2_runner_on_failed(self, result, ignore_errors=False):
        ...
        res_info_list, task_name = self._build_res_info(result)
        self.progress_manager.on_task_failed(task_name, "\n".join(res_info_list))
