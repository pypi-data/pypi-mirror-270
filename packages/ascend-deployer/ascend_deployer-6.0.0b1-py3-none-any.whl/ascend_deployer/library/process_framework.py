...
class Installation:
    ...
    def do_install_mindspore(self):
        ...
        if ok and output:
            self.messages.append("[ASCEND] mindspore is already installed, mindspore install skipped")
            return

        # check python version
        if "3.10." in self.python_version:
            self.messages.append("[ASCEND] mindspore does not support python3.10.* , mindspore install skipped")
            return
    ...
    def do_install_pytorch(self):
        ...
        command = "python3 -m pip install %s --no-index --find-links %s/pylibs" % (run_file_torch_npu, self.resource_dir)
        self.run_command(command)
    ...
    def do_install_tensorflow(self):
        ...
        if ok and output:
            self.messages.append("[ASCEND] tensorflow is already installed, tensorflow install skipped")
            return

        if "3.10." in self.python_version:
            self.messages.append("[ASCEND] tensorflow does not support python3.10.* , tensorflow install skipped")
            return
        ...
        # do install
        command = "python3 -m pip install %s --no-index --find-links %s/pylibs" % (install_tensorflow, self.resource_dir)
        self.run_command(command)
        ok, output = self.run_command(check_tensorflow_cmd)
