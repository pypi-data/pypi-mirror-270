...
def main():
    ...
    parser.add_argument("-cc", "--custom-compiler", help="自作コンパイラー（自作定義書生成文書）")  # 追加: カスタムコンパイラーのオプション
    ...
    if args.input is None:
        ...
        print("例3: zoltraak \"タクシーの経営課題を解決するための戦略ドキュメントを作成する\"  -cc my_def.md")  # 追加: カスタムコンパイラーの使用例
        return
    ...
    if args.input.endswith(".md") or \
       ...
       os.path.isdir(args.input):                                          # 入力がマークダウンファイル、ファイルパス、またはディレクトリパスかどうかを判定
        ...
        if args.custom_compiler:  # カスタムコンパイラーが指定された場合
            if not os.path.isfile(args.custom_compiler):
                print(f"エラー: カスタムコンパイラーのファイル '{args.custom_compiler}' が見つかりません。")
                return
            compiler_path = args.custom_compiler
        else:
