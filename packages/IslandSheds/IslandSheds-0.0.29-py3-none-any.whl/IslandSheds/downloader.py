...
def download_watershed(geopackage_url, country, download_dir="./Watershed/", target_crs=None, watershed=None):
    ...
    # Display available watersheds in the country if no specific watershed is chosen
    if not watershed:
        print(f"Available watersheds in {country}:")
        unique_watersheds = country_gdf['Watershed'].unique()
        for ws in unique_watersheds:
            print(ws)
        return  # Exit the function after displaying watersheds
