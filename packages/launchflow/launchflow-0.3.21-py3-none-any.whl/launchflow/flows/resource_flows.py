...
from launchflow.context.launchflow_ctx import AsyncOp, AsyncResourceNoOp
...
from launchflow.postgres import GenericResource
...
async def stop_local_containers(
    ...
    with Progress(
        ...
        to_stop_options = [container for container in containers if container.container.status == "running"]
        progress.remove_task(task)
    ...
    if prompt:
        ...
        for answer in answers:
            rich.print(
                f"[pink1]>[/pink1] Stop: [blue]{to_stop_options[answer]}[/blue]"
            )
    ...
    if failures:
        progress.console.print(
            f"[red]✗[/red] Failed to stop {failures} containers"
        )
