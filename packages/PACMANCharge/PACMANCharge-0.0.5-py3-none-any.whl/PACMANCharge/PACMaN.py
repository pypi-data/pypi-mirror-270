...
def predict(cif_file,model_name,charge_type,digits,atom_type,neutral):
    if model_name == "COF":
        charge_model_name = resource_filename('PACMANCharge', 'cof.pth')
        nor_name = resource_filename('PACMANCharge', 'cof.pkl')
    elif model_name == "MOF":
        if charge_type == "DDEC":
            charge_model_name = resource_filename('PACMANCharge', 'mof-ddec.pth')
            nor_name = resource_filename('PACMANCharge', 'mof-ddec.pkl')
    ...
    try:
        ...
        for _, (input) in enumerate(loader):
            with torch.no_grad():
                ...
                write4cif(cif_file,chg,digits=digits,atom_type=atom_type,neutral=neutral)
    except Exception as e:
        print(e)