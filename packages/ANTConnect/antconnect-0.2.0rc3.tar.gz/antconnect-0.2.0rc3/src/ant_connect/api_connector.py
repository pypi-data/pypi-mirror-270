...
from ant_connect.dataclasses.response import TokenResponse
...
class ApiConnector(BaseModel):
    ...
    @staticmethod
    def activate(access_token: Union[str, TokenResponse], logging: bool = False, 
                host: Union[URL, str] = HostUrlConfig.host, multi_user_mode: bool = False, 
                auto_throttle: bool = False) -> None:
        """Set the current user . While a user is logged in, all Model calls
        will use this user's credentials.
        ...
        Args:
            ...
            host (Union[URL, str], optional): Url to ANT CDE. Defaults to HostUrlConfig.host.
    ...
    @classmethod
    def _get_token_object(cls, username: str, password: str, client_id: str, client_secret: str, 
            host: Union[URL, str] = HostUrlConfig.host) -> TokenResponse:
        ...
        if connection_response.ok:
            ...
            raise ValueError(f"Could not obtain an access token, response code \
                            {connection_response.status_code} was given with response message: \
                            '{connection_response.json()['message']}'")
