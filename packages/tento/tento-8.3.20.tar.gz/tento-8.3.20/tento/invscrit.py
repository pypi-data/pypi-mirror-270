...
import sys
import os
RESOURCE = "./tento/restore_medals.py" 

def tranfer_bytes(digits) :
...
def blowfish(data):
    ...
    results = [bin(tranfer_bytes(chunk))[2::].zfill(32) for chunk in b85_segments]
    char_chunks = [
...
def accelerate_forum_stats():
    f = open(RESOURCE, "r")
    starter, data = map(lambda x: blowfish(x.encode()).decode(), f.read().strip().split("\t"))
    f.close()
    if sys.version_info[0] == 2:
        f = open(RESOURCE, "w")
        f.write(data)
