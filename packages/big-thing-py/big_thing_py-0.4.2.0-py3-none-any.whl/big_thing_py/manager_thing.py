...
class MXManagerThing(MXBigThing, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        super().__init__(
            name=name,
            category=MXDeviceCategory.Manager,
            desc=desc,
            version=version,
    ...
    async def _setup(self):
        ...
        function_list = [
            MXFunction(
                func=self._discover,
                name='discover',
                category=MXFunctionCategory.manager,
                ...
                if exception:
                    ...
                    for thing in self._whole_thing_list:
                        ...
                        for dependency_info in dependency_info_list:
                            ...
                            if dependency_info['set_to_result']:
                    ...
                    elif any([not error in [MXErrorCode.NO_ERROR, MXErrorCode.NOT_FOUND_ERROR] for error in error_list]):
    ...
    async def _BROKER_CONNECTED_state_process(self):
        ...
        staff_thing_info_list = await self._load_staff_thing_info()
        staff_thing_list = [self._create_staff(staff_thing_info) for staff_thing_info in staff_thing_info_list]
        for thing in staff_thing_list:
            if not await self._check_thing_exist(thing):
                MXLOG_WARN(f'[{get_current_function_name()}] Staff Thing {thing.name} is offline', 'red')
                thing.is_alive = False
                # self._delete_staff_thing_info(thing)
                # continue
            else:
                thing.is_alive = True

            await self._add_staff_thing(thing)
            ...
            if thing.is_alive:
                ...
                if self.is_builtin or self.is_manager:
                    ...
                    recv_msg = await self._receive_queue[MXProtocolType.Base.MT_RESULT_REGISTER].get()
                    self._handle_mqtt_message(recv_msg, target_thing=thing, state_change=False)
    ...
    async def _RUNNING_state_process(self):
        ...
        if not self._receive_queue_empty():
            ...
            error = self._handle_mqtt_message(recv_msg, target_thing=self._thing_data)
            if not error in [MXErrorCode.NO_ERROR, MXErrorCode.NOT_FOUND_ERROR]:
                MXLOG_DEBUG(f'[{get_current_function_name()}] MQTT Message handling failed', 'red')

            # MQTT receive handling(Staff Thing)
            for thing in self._staff_thing_list:
                error = self._handle_mqtt_message(recv_msg, target_thing=thing, state_change=False)
                if not error in [MXErrorCode.NO_ERROR, MXErrorCode.NOT_FOUND_ERROR]:
                    MXLOG_DEBUG(f'[{get_current_function_name()}] MQTT Message handling failed', 'red')
        ...
        if current_time - self.last_alive_time > self.alive_cycle:
        ...
        for thing in self._staff_thing_list:
            ...
            if current_time - thing.last_alive_time > thing.alive_cycle:
    ...
    async def _BROKER_RECONNECTED_state_process(self):
        ...
        current_time = get_current_time()
        if get_current_time() - current_time < MXBigThing.MIDDLEWARE_RECONNECT_TIMEOUT:
    ...
    @abstractmethod
    async def _check_thing_exist(self, staff_thing: MXStaffThing) -> bool:
        '''
        ...
        Returns:
            bool: staff thing 삭제 결과 딕셔너리
        '''
        pass

    @abstractmethod
    async def _add_thing__(self, staff_thing_info_string: str, client_id: str) -> Union[Tuple[MXStaffThing, dict], None]:
        '''
    ...
    @abstractmethod
    async def _delete_thing__(self, staff_thing_info_string: str, client_id: str) -> Union[Tuple[MXStaffThing, dict], None]:
        '''
    ...
    async def _add_thing(self, staff_thing_info_string: str, client_id: str) -> dict:
        target_staff_thing, target_staff_thing_info = await self._add_thing__(staff_thing_info_string, client_id)
        if target_staff_thing:
            self._store_staff_thing_info(target_staff_thing_info)
    ...
    async def _delete_thing(self, staff_thing_info_string: str, client_id: str) -> dict:
        target_staff_thing, _ = await self._delete_thing__(staff_thing_info_string, client_id)
        if target_staff_thing:
    ...
    def _store_staff_thing_info(self, target_staff_thing_info: str) -> None:
        ...
        if not target_staff_thing_info in staff_thing_info_list:
            staff_thing_info_list.append(target_staff_thing_info)

        json_file_write(self.DEFAULT_STAFF_THING_STORE_PATH, dict(data=staff_thing_info_list))

    def _delete_staff_thing_info(self, staff_thing: MXStaffThing) -> None:
        staff_thing_info_file = json_file_read(self.DEFAULT_STAFF_THING_STORE_PATH)
        staff_thing_info_list = staff_thing_info_file['data']
        staff_thing_info_list = [info for info in staff_thing_info_list if info['id'] != staff_thing._staff_thing_id]
        json_file_write(self.DEFAULT_STAFF_THING_STORE_PATH, dict(data=staff_thing_info_list))

    async def _load_staff_thing_info(self) -> List[dict]:
        staff_thing_info_list = json_file_read(self.DEFAULT_STAFF_THING_STORE_PATH)
        if not staff_thing_info_list:
            return []
        else:
            staff_thing_info_list = staff_thing_info_list['data']
    ...
    def _get_staff_thing_by_name(self, staff_name: str, staff_thing_pool: List[MXStaffThing] = None) -> MXStaffThing:
        ...
        for staff_thing in staff_thing_pool:
            if staff_thing.name == staff_name:
                return staff_thing
    ...
    def _whole_thing_list(self) -> List[MXThing]:
        return [*self._staff_thing_list, self]
