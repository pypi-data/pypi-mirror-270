...
class MXBigThing:
    ...
    def __init__(
        ...
        category: MXDeviceCategory = MXThing.DEFAULT_CATEGORY,
        desc='',
        version=sdk_version(),
        ...
        self._ble_advertiser = BLEAdvertiser(server_name='MySSIX Thing BLE Server')
    ...
    async def _setup(self) -> 'MXBigThing':
        ...
        self._mqtt_mdns_service = ServiceInfo(
            ...
            name='_myssix_thing._mqtt._tcp.local.',
        ...
        self._mqtt_ssl_mdns_service = ServiceInfo(
            type_='_mqtt._tcp.local.',
            name='_ssl_myssix_thing._mqtt._tcp.local.',
            addresses=[socket.inet_aton(get_local_ip())],
            port=8883,
    ...
    async def _RESET_state_process(self):
        if self.is_builtin or self.is_manager:
            if self.is_builtin:
                self.next_state = ThingState.BUILTIN_SETUP
            elif self.is_manager:
                self.next_state = ThingState.PLUGIN_SETUP
        else:
    ...
    async def _BUILTIN_SETUP_state_process(self):
        if not await self._connect_to_broker(MXBigThing.CONNECT_RETRY):
    ...
    async def _PLUGIN_SETUP_state_process(self):
        if not await self._connect_to_broker(MXBigThing.CONNECT_RETRY):
            self.next_state = ThingState.SHUTDOWN
            return
    ...
    async def _main_event_loop(self):
        while not self._g_exit.is_set():
            try:
                ...
                elif self._state == ThingState.BUILTIN_SETUP:
                    await self._BUILTIN_SETUP_state_process()
                elif self._state == ThingState.PLUGIN_SETUP:
                    await self._PLUGIN_SETUP_state_process()
    ...
    def _handle_mqtt_message(self, msg: MQTTMessage, target_thing: MXThing, state_change: bool = True) -> bool:
        ...
        if protocol == MXProtocolType.Base.MT_REQUEST_REGISTER_INFO:
            ...
            if self.name == target_thing.name:
    ...
    def _handle_MT_RESULT_REGISTER(self, msg: MQTTMessage, target_thing: MXThing) -> MXErrorCode:
        ...
        if error in [MXErrorCode.NO_ERROR, MXErrorCode.DUPLICATE]:
            MXLOG_DEBUG(f'{PrintTag.GOOD if error == MXErrorCode.NO_ERROR else PrintTag.DUP} Thing {target_thing.name} register success!')
    ...
    def _handle_MT_RESULT_UNREGISTER(self, msg: MQTTMessage, target_thing: MXThing) -> MXErrorCode:
        ...
        if error in [MXErrorCode.NO_ERROR, MXErrorCode.DUPLICATE]:
            MXLOG_DEBUG(f'{PrintTag.GOOD if error == MXErrorCode.NO_ERROR else PrintTag.DUP} Thing {target_thing.name} unregister success!')
    ...
    def _send_TM_ALIVE(self, thing_data: MXThing):
        ...
        thing_data.last_alive_time = get_current_time()
    ...
    def add_dependency(self, function: MXFunction, value: MXValue) -> None:
        self._thing_data.add_dependency(function, value)
    ...
    async def _connect_to_broker(self, connect_try: int) -> bool:
        ...
        while connect_try:
            try:
                await self._connect(self._ip, self._port, ssl=ssl_context)
                if self._mqtt_client.is_connected:
                    return True
    ...
    async def _connect(self, ip: str, port: int, ssl: Union[ssl.SSLContext, bool]) -> bool:
        ...
        await self._mqtt_client.connect(ip, port, ssl=ssl)
