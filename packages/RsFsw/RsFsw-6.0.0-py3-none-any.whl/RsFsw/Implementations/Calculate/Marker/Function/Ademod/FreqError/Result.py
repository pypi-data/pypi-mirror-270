...
class ResultCls:
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default, trace=repcap.Trace.Default) -> float:
		...
		Queries the carrier offset (= frequency error) for FM and PM demodulation. The carrier offset is determined from the
		current measurement data (CLR/WRITE) . The modulation is removed using low pass filtering. \n
