...
class UnitCls:
	...
	def set(self, unit: enums.DiqUnit) -> None:
		...
		Snippet: driver.applications.k10Xlte.inputPy.diq.range.upper.unit.set(unit = enums.DiqUnit.AMPere) \n
		...
		Is only available if the optional 'Digital Baseband' interface is installed. \n
			:param unit: DBM | DBPW | WATT | DBUV | DBMV | VOLT | DBUA | AMPere
		"""
	...
	def get(self) -> enums.DiqUnit:
		"""SCPI: INPut:DIQ:RANGe[:UPPer]:UNIT \n
		Snippet: value: enums.DiqUnit = driver.applications.k10Xlte.inputPy.diq.range.upper.unit.get() \n
		Defines the unit of the full scale level. The availability of units depends on the measurement application you are using.
		Is only available if the optional 'Digital Baseband' interface is installed. \n
