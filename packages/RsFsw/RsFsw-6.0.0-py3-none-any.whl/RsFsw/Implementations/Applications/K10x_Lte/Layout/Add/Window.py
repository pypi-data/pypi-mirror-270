...
class WindowCls:
	...
	def get(self, window_name: str, direction: enums.WindowDirection, window_type: enums.WindowTypeK10X) -> str:
		...
		Snippet: value: str = driver.applications.k10Xlte.layout.add.window.get(window_name = 'abc', direction = enums.WindowDirection.ABOVe, window_type = enums.WindowTypeK10X.AllocIdVsSymbolXcarrier=AISC) \n
		Adds a window to the display in the active channel. Is always used as a query so that you immediately obtain the name of
		the new window as a result. To replace an existing window, use the method RsFsw.Layout.Replace.Window.set command. Note:
		Use this command to select a result display instead of method RsFsw.Calculate.Feed.set (still supported for compatibility
		reasons, but deprecated) . \n
