...
class SelectCls:
	...
	def set(self, select: str) -> None:
		...
		Snippet: driver.applications.k9X11Ad.sense.correction.cvl.select.set(select = 'abc') \n
	...
	def get(self, select: str) -> str:
		"""SCPI: [SENSe]:CORRection:CVL:SELect \n
		Snippet: value: str = driver.applications.k9X11Ad.sense.correction.cvl.select.get(select = 'abc') \n
		Selects the conversion loss table with the specified file name. If <file_name> is not available, a new conversion loss
		table is created. Is only available with option B21 (External Mixer) installed. \n
			:param select: String containing the path and name of the file.
