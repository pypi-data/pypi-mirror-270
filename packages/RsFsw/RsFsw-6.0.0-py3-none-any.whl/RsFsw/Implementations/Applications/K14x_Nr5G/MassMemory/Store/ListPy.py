...
class ListPyCls:
	...
	def set(self, filename: str, store=repcap.Store.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.massMemory.store.listPy.set(filename = 'abc', store = repcap.Store.Default) \n
		Exports the SEM and spurious emission list evaluation to a file. The file format is *.dat. Secure User Mode In secure
		user mode, settings that are stored on the instrument are stored to volatile memory, which is restricted to 256 MB. Thus,
		a 'memory limit reached' error can occur although the hard disk indicates that storage space is still available. To store
