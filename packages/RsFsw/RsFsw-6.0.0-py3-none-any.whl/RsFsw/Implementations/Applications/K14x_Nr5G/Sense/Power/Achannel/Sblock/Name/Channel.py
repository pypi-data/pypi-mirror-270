...
class ChannelCls:
	...
	def set(self, name: str, subBlock=repcap.SubBlock.Default, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.sense.power.achannel.sblock.name.channel.set(name = 'abc', subBlock = repcap.SubBlock.Default, channel = repcap.Channel.Default) \n
		Defines the name of the specified MSR Tx channel. Is for MSR signals only. In MSR ACLR measurements, the default TX
		channel names correspond to the specified technology, followed by a consecutive number. The assigned sub block (A,B,C,D,E,
		F,G,H) is indicated as a prefix (e.g. A: WCDMA1) . Is for MSR signals only (see method RsFsw.Calculate.Marker.Function.
