...
class WindowCls:
	"""Window commands group definition. 31 total commands, 8 Subgroups, 0 group commands
	...
	@property
	def subwindow(self):
		"""subwindow commands group. 8 Sub-classes, 0 commands."""
	...
	def tab(self):
		...
		if not hasattr(self, '_tab'):
			from .Tab import TabCls
			self._tab = TabCls(self._core, self._cmd_group)
		return self._tab
	...
	def tpoo(self):
		...
		if not hasattr(self, '_tpoo'):
			from .Tpoo import TpooCls
			self._tpoo = TpooCls(self._core, self._cmd_group)
		return self._tpoo
