...
class RelativeCls:
	...
	def set(self, sb_gaps: enums.SubBlockGaps, limit: float, window=repcap.Window.Default, limitIx=repcap.LimitIx.Default, gapChannel=repcap.GapChannel.Default) -> None:
		...
		Defines the relative limit for the ACLR power in the specified lower gap channel. The reference value for the relative
		limit is the measured channel power. If you define both an absolute limit and a relative limit, the FSW uses the lower
		value for the limit check. Is only available for for asymmetrical (manual) configuration of gap channels (see
