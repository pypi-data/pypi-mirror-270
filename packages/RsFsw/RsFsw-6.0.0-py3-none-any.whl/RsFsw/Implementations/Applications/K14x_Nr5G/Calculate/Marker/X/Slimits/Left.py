...
class LeftCls:
	...
	def set(self, search_limit: float, window=repcap.Window.Default, marker=repcap.Marker.Default) -> None:
		...
		time domain, this command limits the range of the trace to be analyzed. \n
			:param search_limit: The value range depends on the frequency range or measurement time. The unit is Hz for frequency domain measurements and s for time domain measurements. Unit: HZ
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default) -> float:
		...
		time domain, this command limits the range of the trace to be analyzed. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:param marker: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Marker')
			:return: search_limit: The value range depends on the frequency range or measurement time. The unit is Hz for frequency domain measurements and s for time domain measurements. Unit: HZ"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		marker_cmd_val = self._cmd_group.get_repcap_cmd_value(marker, repcap.Marker)
