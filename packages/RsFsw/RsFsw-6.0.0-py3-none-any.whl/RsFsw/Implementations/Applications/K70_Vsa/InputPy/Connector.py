...
class ConnectorCls:
	...
	def set(self, input_connector: enums.InputConnectorB) -> None:
		"""SCPI: INPut:CONNector \n
		Snippet: driver.applications.k70Vsa.inputPy.connector.set(input_connector = enums.InputConnectorB.AIQI) \n
		...
		and Providing Data Output'. If an external frontend is active, the connector is automatically set to RF. \n
			:param input_connector: RF RF input connector AIQI Analog Baseband I connector This setting is only available if the 'Analog Baseband' interface (FSW-B71) is installed and active for input. It is not available for the FSW67 or FSW85. RFPRobe Active RF probe
		"""
	...
	def get(self) -> enums.InputConnectorB:
		"""SCPI: INPut:CONNector \n
		Snippet: value: enums.InputConnectorB = driver.applications.k70Vsa.inputPy.connector.get() \n
		Determines which connector the input for the measurement is taken from. For more information, see 'Receiving Data Input
		and Providing Data Output'. If an external frontend is active, the connector is automatically set to RF. \n
