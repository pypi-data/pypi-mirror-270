...
class SkipCls:
	...
	def rising(self):
		...
		if not hasattr(self, '_rising'):
			from .Rising import RisingCls
			self._rising = RisingCls(self._core, self._cmd_group)
		return self._rising
	...
	def falling(self):
		...
		if not hasattr(self, '_falling'):
			from .Falling import FallingCls
			self._falling = FallingCls(self._core, self._cmd_group)
		return self._falling

	def clone(self) -> 'SkipCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = SkipCls(self._core, self._cmd_group.parent)
