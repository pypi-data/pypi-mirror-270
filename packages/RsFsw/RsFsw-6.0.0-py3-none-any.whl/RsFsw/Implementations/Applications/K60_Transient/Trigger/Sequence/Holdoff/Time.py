...
class TimeCls:
	...
	def set(self, offset: float) -> None:
		...
		Snippet: driver.applications.k60Transient.trigger.sequence.holdoff.time.set(offset = 1.0) \n
	...
	def get(self) -> float:
		"""SCPI: TRIGger[:SEQuence]:HOLDoff[:TIME] \n
		Snippet: value: float = driver.applications.k60Transient.trigger.sequence.holdoff.time.get() \n
		Defines the time offset between the trigger event and the start of the sweep. \n
