...
class SwitchingCls:
	...
	def set(self, state: bool, scaling: enums.TimeScaling = None, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k60Transient.calculate.chrDetection.table.timing.switching.set(state = False, scaling = enums.TimeScaling.MS, window = repcap.Window.Default) \n
		...
		self._core.io.write(f'CALCulate{window_cmd_val}:CHRDetection:TABLe:TIMing:SWITching {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> SwitchingStruct:
		"""SCPI: CALCulate<n>:CHRDetection:TABLe:TIMing:SWITching \n
		Snippet: value: SwitchingStruct = driver.applications.k60Transient.calculate.chrDetection.table.timing.switching.get(window = repcap.Window.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:return: structure: for return value, see the help for SwitchingStruct structure arguments."""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'CALCulate{window_cmd_val}:CHRDetection:TABLe:TIMing:SWITching?', self.__class__.SwitchingStruct())
