...
class ApertureCls:
	...
	def set(self, ref_value: float, window=repcap.Window.Default, trace=repcap.Trace.Default) -> None:
		...
		Snippet: driver.applications.k17Mcgd.display.window.trace.smoothing.aperture.set(ref_value = 1.0, window = repcap.Window.Default, trace = repcap.Trace.Default) \n
	...
	def get(self, window=repcap.Window.Default, trace=repcap.Trace.Default) -> float:
		"""SCPI: DISPlay[:WINDow<n>]:TRACe<t>:SMOothing:APERture \n
		Snippet: value: float = driver.applications.k17Mcgd.display.window.trace.smoothing.aperture.get(window = repcap.Window.Default, trace = repcap.Trace.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:param trace: optional repeated capability selector. Default value: Tr1 (settable in the interface 'Trace')
