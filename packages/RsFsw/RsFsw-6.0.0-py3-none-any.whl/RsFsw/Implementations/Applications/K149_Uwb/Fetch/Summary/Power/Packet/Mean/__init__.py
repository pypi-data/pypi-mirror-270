...
class MeanCls:
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:SUMMary:POWer:PACKet:MEAN \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.summary.power.packet.mean.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:POWer:PACKet:MEAN?')
		return Conversions.str_to_float(response)

	def clone(self) -> 'MeanCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = MeanCls(self._core, self._cmd_group.parent)
