...
class ApacketsCls:
	...
	def get(self, window=repcap.Window.Default) -> str:
		"""SCPI: FETCh<n>:PACKet:SYNC:SFD:LENGth:APACkets \n
		Snippet: value: str = driver.applications.k149Uwb.fetch.packet.sync.sfd.length.apackets.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:SYNC:SFD:LENGth:APACkets?')
