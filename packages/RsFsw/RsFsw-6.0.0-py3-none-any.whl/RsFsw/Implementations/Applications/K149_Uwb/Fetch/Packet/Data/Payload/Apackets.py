...
class ApacketsCls:
	...
	def get(self, window=repcap.Window.Default) -> str:
		"""SCPI: FETCh<n>:PACKet:DATA:PAYLoad:APACkets \n
		Snippet: value: str = driver.applications.k149Uwb.fetch.packet.data.payload.apackets.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:DATA:PAYLoad:APACkets?')
