...
class LevelCls:
	...
	def set(self, peak_voltage: float) -> None:
		...
		Snippet: driver.applications.k18AmplifierEt.inputPy.iq.fullscale.level.set(peak_voltage = 1.0) \n
	...
	def get(self) -> float:
		"""SCPI: INPut:IQ:FULLscale:LEVel \n
		Snippet: value: float = driver.applications.k18AmplifierEt.inputPy.iq.fullscale.level.get() \n
		Defines the peak voltage at the Baseband Input connector if the full scale level is set to manual mode (see method RsFsw.
		Applications.K10x_Lte.InputPy.Iq.Fullscale.Auto.set) . \n
