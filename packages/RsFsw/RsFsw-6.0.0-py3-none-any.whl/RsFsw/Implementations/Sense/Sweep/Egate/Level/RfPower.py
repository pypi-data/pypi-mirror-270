...
class RfPowerCls:
	...
	def set(self, gate_level: float) -> None:
		...
		500 MHz and 8 GHz. Is only available for triggered gated measurements ([SENSe:]SWEep:EGATe:AUTOMAN) . \n
			:param gate_level: For details on available trigger levels and trigger bandwidths, refer to the specifications document. Unit: DBM
		"""
	...
	def get(self) -> float:
		...
		the trigger level is analyzed. If defined, a reference level offset is also considered. The input signal must be between
		500 MHz and 8 GHz. Is only available for triggered gated measurements ([SENSe:]SWEep:EGATe:AUTOMAN) . \n
			:return: gate_level: For details on available trigger levels and trigger bandwidths, refer to the specifications document. Unit: DBM"""
