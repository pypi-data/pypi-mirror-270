...
class LeftCls:
	...
	def set(self, window=repcap.Window.Default, deltaMarker=repcap.DeltaMarker.Default, opc_timeout_ms: int = -1) -> None:
		...
		Moves a delta marker to the next minimum peak value. The search includes only measurement values to the right of the
		current marker position. In the spectrogram, the command moves a marker horizontally to the minimum level in the
