...
class ResultCls:
	...
	def get(self, meas_type: enums.AdemMeasType, window=repcap.Window.Default, marker=repcap.Marker.Default, trace=repcap.Trace.Default) -> float:
		...
		Queries the current value of the demodulated signal for the specified trace (as displayed in the 'Result Summary' in
		manual operation) . Note that all windows with the same evaluation method have the same traces, thus the window is
		irrelevant. \n
