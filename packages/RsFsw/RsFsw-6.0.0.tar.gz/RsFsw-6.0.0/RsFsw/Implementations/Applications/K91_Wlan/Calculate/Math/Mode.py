...
class ModeCls:
	...
	def set(self, mode: enums.AverageModeA, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k91Wlan.calculate.math.mode.set(mode = enums.AverageModeA.LINear, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> enums.AverageModeA:
		"""SCPI: CALCulate<n>:MATH:MODE \n
		Snippet: value: enums.AverageModeA = driver.applications.k91Wlan.calculate.math.mode.get(window = repcap.Window.Default) \n
		Selects the way the FSW calculates trace mathematics. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
