...
class SwitchingCls:
	...
	def get(self, query_range: enums.SelectionRange) -> float:
		"""SCPI: [SENSe]:CHIRp:TIMing:SWITching \n
		Snippet: value: float = driver.applications.k60Transient.sense.chirp.timing.switching.get(query_range = enums.SelectionRange.ALL) \n
		...
		response = self._core.io.query_str(f'SENSe:CHIRp:TIMing:SWITching? {param}')
