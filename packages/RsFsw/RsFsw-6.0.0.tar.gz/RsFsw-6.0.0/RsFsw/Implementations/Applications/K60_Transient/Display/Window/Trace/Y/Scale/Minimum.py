...
class MinimumCls:
	...
	def set(self, value: float, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k60Transient.display.window.trace.y.scale.minimum.set(value = 1.0, window = repcap.Window.Default) \n
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: DISPlay[:WINDow<n>]:TRACe:Y[:SCALe]:MINimum \n
		Snippet: value: float = driver.applications.k60Transient.display.window.trace.y.scale.minimum.get(window = repcap.Window.Default) \n
		Defines the minimum value on the y-axis in the specified window. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
