...
class FilterPyCls:
	...
	def yig(self):
		...
		if not hasattr(self, '_yig'):
			from .Yig import YigCls
			self._yig = YigCls(self._core, self._cmd_group)
		return self._yig
	...
	@property
	def saw(self):
		"""saw commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_saw'):
			from .Saw import SawCls
			self._saw = SawCls(self._core, self._cmd_group)
		return self._saw
