...
class AutoCls:
	...
	def get(self) -> bool:
		...
		By default, the attenuation settings are applied at the input of the external frontend, see method RsFsw.Applications.
		K10x_Lte.InputPy.Attenuation.Auto.set and method RsFsw.Applications.K10x_Lte.InputPy.Attenuation.set. \n
			:return: state: ON | OFF | 0 | 1 OFF | 0 Auto mode for analyzer attenuation is disabled. Allows you to configure attenuation at the analyzer using method RsFsw.InputPy.Sanalyzer.Attenuation.set. ON | 1 Auto mode for analyzer attenuation is enabled. No attenuation is configured at the analyzer."""
		response = self._core.io.query_str(f'INPut:SANalyzer:ATTenuation:AUTO?')
