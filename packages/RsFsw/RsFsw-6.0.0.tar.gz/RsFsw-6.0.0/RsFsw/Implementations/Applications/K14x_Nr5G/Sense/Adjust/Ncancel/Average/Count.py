...
class CountCls:
	...
	def set(self, length: float) -> None:
		...
		Snippet: driver.applications.k14Xnr5G.sense.adjust.ncancel.average.count.set(length = 1.0) \n
		...
		. \n
			:param length: integer Number of measurements
		"""
	...
	def get(self) -> float:
		"""SCPI: [SENSe]:ADJust:NCANcel:AVERage:COUNt \n
		Snippet: value: float = driver.applications.k14Xnr5G.sense.adjust.ncancel.average.count.get() \n
		Defines the number of measurements that are performed on the captured I/Q data to determine the average noise density due
		to the spectrum analyzer. Only available if I/Q noise cancellation is enabled ([SENSe:]ADJust:NCANcel:AVERage[:STATe] ON)
		. \n
			:return: length: integer Number of measurements"""
