...
class RefLevelCls:
	...
	def set(self, reference_level: float, window=repcap.Window.Default, subWindow=repcap.SubWindow.Default, trace=repcap.Trace.Default) -> None:
		...
		Defines the reference level (for all traces in all windows) . With a reference level offset != 0, the value range of the
		reference level is modified by the offset. \n
			:param reference_level: The unit is variable. Range: see specifications document , Unit: DBM
	...
	def get(self, window=repcap.Window.Default, subWindow=repcap.SubWindow.Default, trace=repcap.Trace.Default) -> float:
		...
		reference level is modified by the offset. \n
			...
			:param subWindow: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Subwindow')
			:param trace: optional repeated capability selector. Default value: Tr1 (settable in the interface 'Trace')
			:return: reference_level: The unit is variable. Range: see specifications document , Unit: DBM"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		subWindow_cmd_val = self._cmd_group.get_repcap_cmd_value(subWindow, repcap.SubWindow)
		trace_cmd_val = self._cmd_group.get_repcap_cmd_value(trace, repcap.Trace)
