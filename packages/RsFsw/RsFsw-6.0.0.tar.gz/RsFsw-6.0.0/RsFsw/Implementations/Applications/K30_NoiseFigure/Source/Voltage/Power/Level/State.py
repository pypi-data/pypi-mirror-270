...
class StateCls:
	...
	def set(self, state: bool, source=repcap.Source.Default) -> None:
		...
		Snippet: driver.applications.k30NoiseFigure.source.voltage.power.level.state.set(state = False, source = repcap.Source.Default) \n
	...
	def get(self, source=repcap.Source.Default) -> bool:
		"""SCPI: SOURce:VOLTage:POWer<i>:LEVel[:STATe] \n
		Snippet: value: bool = driver.applications.k30NoiseFigure.source.voltage.power.level.state.get(source = repcap.Source.Default) \n
		No command help available \n
			:param source: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Power')
