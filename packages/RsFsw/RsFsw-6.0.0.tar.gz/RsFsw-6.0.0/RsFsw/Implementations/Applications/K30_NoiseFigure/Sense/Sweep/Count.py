...
class CountCls:
	...
	def set(self, averages: float) -> None:
		...
		measurement and calculates the average after the average count has been reached. \n
			:param averages: If you set a sweep count of 0 or 1, the application performs one single sweep in single sweep mode. In continuous sweep mode, if the average count is set to 0, a moving average over 10 sweeps is performed. Range: 0 to 32767
		"""
	...
	def get(self) -> float:
		...
		the application calculates the moving average over the average count. In single sweep mode, the application stops the
		measurement and calculates the average after the average count has been reached. \n
			:return: averages: No help available"""
