...
class QcomponentCls:
	...
	def set(self, value: float) -> None:
		...
		Snippet: driver.applications.k149Uwb.inputPy.iq.osc.skew.qcomponent.set(value = 1.0) \n
		Compensates for skewed values in the positive Q path, e.g. due to different input cables. \n
			:param value: Unit: S
		"""
		param = Conversions.decimal_value_to_str(value)
		self._core.io.write(f'INPut:IQ:OSC:SKEW:Q {param}')

	def get(self) -> float:
		"""SCPI: INPut:IQ:OSC:SKEW:Q \n
		Snippet: value: float = driver.applications.k149Uwb.inputPy.iq.osc.skew.qcomponent.get() \n
		Compensates for skewed values in the positive Q path, e.g. due to different input cables. \n
			:return: value: Unit: S"""
		response = self._core.io.query_str(f'INPut:IQ:OSC:SKEW:Q?')
