...
class LengthCls:
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: FETCh<n>:PACKet:SYNC:SFD:LENGth \n
		Snippet: value: float = driver.applications.k149Uwb.fetch.packet.sync.sfd.length.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:SYNC:SFD:LENGth?')
