...
class SecdedCls:
	"""Secded commands group definition. 2 total commands, 1 Subgroups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("secded", core, parent)
	...
	def get(self, window=repcap.Window.Default) -> str:
		"""SCPI: FETCh<n>:PACKet:DATA:SECDed \n
		Snippet: value: str = driver.applications.k149Uwb.fetch.packet.data.secded.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:PACKet:DATA:SECDed?')
		return trim_str_response(response)

	def clone(self) -> 'SecdedCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = SecdedCls(self._core, self._cmd_group.parent)
