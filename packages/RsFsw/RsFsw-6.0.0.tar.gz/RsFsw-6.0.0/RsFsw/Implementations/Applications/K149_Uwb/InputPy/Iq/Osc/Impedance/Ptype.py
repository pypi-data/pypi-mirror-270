...
class PtypeCls:
	...
	def set(self, pad_type: enums.PadType) -> None:
		...
		Snippet: driver.applications.k149Uwb.inputPy.iq.osc.impedance.ptype.set(pad_type = enums.PadType.MLPad) \n
		...
		self._core.io.write(f'INPut:IQ:OSC:IMPedance:PTYPe {param}')

	# noinspection PyTypeChecker
	def get(self) -> enums.PadType:
		"""SCPI: INPut:IQ:OSC:IMPedance:PTYPe \n
		Snippet: value: enums.PadType = driver.applications.k149Uwb.inputPy.iq.osc.impedance.ptype.get() \n
		Defines the type of matching pad used for impedance conversion for analog baseband input. For RF input, use the method
		RsFsw.InputPy.Impedance.Ptype.set command. For analog baseband input without an oscilloscope, use the method RsFsw.
		InputPy.Iq.Impedance.Ptype.set command. \n
			:return: pad_type: SRESistor | MLPad SRESistor Series-R MLPad Minimum Loss Pad"""
		response = self._core.io.query_str(f'INPut:IQ:OSC:IMPedance:PTYPe?')
