...
class SyncCls:
	"""Sync commands group definition. 4 total commands, 3 Subgroups, 1 group commands"""
	...
	def get(self, window=repcap.Window.Default) -> str:
		"""SCPI: FETCh<n>:SUMMary:PULSe:LOCation:SYNC \n
		Snippet: value: str = driver.applications.k149Uwb.fetch.summary.pulse.location.sync.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'FETCh{window_cmd_val}:SUMMary:PULSe:LOCation:SYNC?')
