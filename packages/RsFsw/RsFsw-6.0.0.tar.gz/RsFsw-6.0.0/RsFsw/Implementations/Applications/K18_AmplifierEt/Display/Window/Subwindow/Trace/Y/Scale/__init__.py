...
class ScaleCls:
	...
	def set(self, level: float, window=repcap.Window.Default, subWindow=repcap.SubWindow.Default, trace=repcap.Trace.Default) -> None:
		...
		Defines the display range of the y-axis (for all traces) . Note that the command works only for a logarithmic scaling.
		You can select the scaling with method RsFsw.Display.Window.Subwindow.Trace.Y.Spacing.set. \n
