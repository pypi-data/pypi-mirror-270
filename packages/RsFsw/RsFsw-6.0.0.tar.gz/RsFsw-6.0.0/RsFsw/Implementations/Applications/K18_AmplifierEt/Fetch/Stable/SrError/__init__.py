...
class SrErrorCls:
	...
	def stDeviation(self):
		...
		if not hasattr(self, '_stDeviation'):
			from .StDeviation import StDeviationCls
			self._stDeviation = StDeviationCls(self._core, self._cmd_group)
	...
	def clone(self) -> 'SrErrorCls':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = SrErrorCls(self._core, self._cmd_group.parent)
