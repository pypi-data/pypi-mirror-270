...
class OtypeCls:
	...
	def set(self, output_type: enums.OutputType, triggerPort=repcap.TriggerPort.Default) -> None:
		"""SCPI: OUTPut:TRIGger<tp>:OTYPe \n
		Snippet: driver.output.trigger.otype.set(output_type = enums.OutputType.DEVice, triggerPort = repcap.TriggerPort.Default) \n
	...
	def get(self, triggerPort=repcap.TriggerPort.Default) -> enums.OutputType:
		"""SCPI: OUTPut:TRIGger<tp>:OTYPe \n
		Snippet: value: enums.OutputType = driver.output.trigger.otype.get(triggerPort = repcap.TriggerPort.Default) \n
		Selects the type of signal generated at the trigger output. Note: For offline AF or RF triggers, no output signal is
		provided. \n
			:param triggerPort: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Trigger')
