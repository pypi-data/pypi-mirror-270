...
impl Compressor {
    ...
    pub fn from_config_kwargs(py: Python, kwargs: Option<&PyDict>) -> Result<Self, PyErr> {
        let mut depythonizer =
            pythonize::Depythonizer::from_object(kwargs.unwrap_or_else(|| PyDict::new(py)));
...
impl ConcreteCompressor {
    ...
    pub fn build<'py>(
        &self,
        py: Python<'py>,
    ) -> Result<Vec<&'py core_compressor::numcodecs::Codec>, PyErr> {
        self.concrete
            .build_py(py)
            .map_err(core_error::LocationError::into_error)
    }
...
pub fn compute_dataarray_compress_decompress<'py>(
    py: Python<'py>,
    da: &'py PyAny,
    compressor: Vec<&'py core_compressor::numcodecs::Codec>,
) -> Result<
    (
        &'py PyAny,
