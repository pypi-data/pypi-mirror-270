use numpy::{PyArray1, PyArrayDyn, PyUntypedArray};
use pyo3::{
...
impl WasmCodec {
    ...
    pub fn encode<'py>(&mut self, py: Python<'py>, buf: &'py PyAny) -> Result<&'py PyAny, PyErr> {
        self.process(
            py,
            buf,
    ...
    pub fn decode<'py>(
        ...
        if let Some(out) = out {
            ...
            self.process(py, buf, codecs_core_host::WasmCodec::decode, class_method)
        }
    }

    pub fn get_config<'py>(&mut self, py: Python<'py>) -> Result<&'py PyDict, PyErr> {
        let json_loads = py
            .import(intern!(py, "json"))?
    ...
    #[classmethod]
    pub fn from_config<'py>(cls: &'py PyType, config: &'py PyDict) -> Result<&'py PyAny, PyErr> {
...
impl WasmCodec {
    fn process<'py>(
        ...
        Self::with_pyarray_as_buffer_slice(py, buf, class_method, |data, shape| {
            ...
            let processed_py = Self::shaped_buffer_into_pyarray(py, processed, class_method)?;

            Ok(processed_py.as_ref())
        })
    }
    ...
    fn with_pyarray_as_buffer_slice<O>(
        py: Python,
        buf: &PyAny,
        class_method: &str,
        ...
        let data: &PyUntypedArray = ensure_ndarray_like.call1((buf,))?.extract()?;
        let dtype = data.dtype();

        if dtype.is_equiv_to(numpy::dtype::<u8>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<u8>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<u16>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<u16>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<u32>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<u32>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<u64>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<u64>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<i8>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<i8>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<i16>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<i16>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<i32>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<i32>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<i64>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<i64>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<f32>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<f32>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<f64>(py)) {
            with_pyarray_as_buffer_slice_inner(data.downcast::<PyArrayDyn<f64>>()?, with)
        } else {
    ...
    fn with_pyarray_as_buffer_slice_mut<O>(
        py: Python,
        buf: &PyAny,
        class_method: &str,
        ...
        if dtype.is_equiv_to(numpy::dtype::<u8>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<u8>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<u16>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<u16>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<u32>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<u32>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<u64>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<u64>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<i8>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<i8>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<i16>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<i16>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<i32>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<i32>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<i64>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<i64>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<f32>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<f32>>()?, with)
        } else if dtype.is_equiv_to(numpy::dtype::<f64>(py)) {
            with_pyarray_as_buffer_slice_mut_inner(data.downcast::<PyArrayDyn<f64>>()?, with)
        } else {
            Err(PyTypeError::new_err(format!(
                "{class_method} received buffer of unsupported dtype `{dtype}`",
            )))
    ...
    fn shaped_buffer_into_pyarray<'py>(
        ...
        match buffer.buffer {
            codecs_core::BufferVec::U8(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
            codecs_core::BufferVec::U16(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
            codecs_core::BufferVec::U32(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
            codecs_core::BufferVec::U64(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
            codecs_core::BufferVec::I8(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
            codecs_core::BufferVec::I16(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
            codecs_core::BufferVec::I32(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
            codecs_core::BufferVec::I64(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
            codecs_core::BufferVec::F32(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
            codecs_core::BufferVec::F64(v) => Ok(PyArray1::from_vec(py, v)
                .reshape(buffer.shape)?
                .as_untyped()),
