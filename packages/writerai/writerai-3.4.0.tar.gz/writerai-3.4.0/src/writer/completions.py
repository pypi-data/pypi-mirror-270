...
from writer.utils import eventstreaming
...
class Completions:
    ...
    def stream(self, completion_request: models.CompletionRequest, model_id: str, organization_id: Optional[int] = None) -> models.StreamResponse:
        r"""Create streaming completion for LLM model"""
        hook_ctx = HookContext(operation_id='stream', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        _globals = models.StreamGlobals(
            organization_id=self.sdk_configuration.globals.organization_id,
        )
        
        base_url = utils.template_url(*self.sdk_configuration.get_server_details())
        
        url = utils.generate_url(base_url, '/llm/organization/{organizationId}/model/{modelId}/completions/stream', request, _globals)
        ...
        headers = { **utils.get_headers(request, _globals), **headers }
        req_content_type, data, form = utils.serialize_request_body(request, models.StreamRequest, "completion_request", False, False, 'json')
        if req_content_type is not None and req_content_type not in ('multipart/form-data', 'multipart/mixed'):
            headers['content-type'] = req_content_type
        if data is None and form is None:
            raise Exception('request body is required')
        ...
        res = models.StreamResponse(status_code=http_res.status_code, content_type=http_res.headers.get('Content-Type') or '', raw_response=http_res, headers=None)
        
        if http_res.status_code == 200:
            res.headers = http_res.headers
            
            if utils.match_content_type(http_res.headers.get('Content-Type') or '', 'text/event-stream'):                
                out = eventstreaming.stream_events(http_res.iter_content(None), lambda raw: utils.unmarshal_json(raw, models.CompletionEvent))
                res.completion_event = out
            else:
                content_type = http_res.headers.get('Content-Type')
                raise models.SDKError(f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
        elif http_res.status_code in [400, 401, 403, 404, 500]:
