...
def make_input_ppi_db(filein, 
    ...
    conn_ppi_db = sqlite3.connect(f"file::memory:?cache=shared",uri=True)
    # off the journal
    ...
    #print(f"Runtime obsdb: {round(tc_1-tc_0,4)} s")
...
def superobs(filein, 
    ...
    query = (  f'select count(*) from data natural join header  WHERE  id_stn = ? and time = ? and nyquist >= ? ;') #ORDER BY RANDOM()  ;')
   # c =   conn_elevation.cursor()
   # avg1  = c.execute(query, (stn, hhmiss_nopad, 0, ))
   # result = c.fetchone()
    ...
    for nominal_ppi_elevation in nominal_ppi_elevations:
        ...
        conn_ppi_db, nobs = make_input_ppi_db(filein, 
                                              ...
                                              obs_nyquist)
        tc_1 = time.time()
        time_schema +=tc_1-tc_0

        print (nominal_ppi_elevation,nobs, hhmiss_nopad, filein)

       # if os.path.exists(model_data_dir): 
        aa=False
      #  print (stn,nominal_ppi_elevation,nobs)
        if aa:
          tc_0 = time.time()
          make_dealias(this_datetime,
                         ...
                         pathoutpkl)
          tc_1 = time.time()
          time_dealias +=tc_1-tc_0

  
        #temporarily attach ppi db to output file
        conn_filememory.execute(f'ATTACH DATABASE "file::memory:?cache=shared" AS db_ppi') 
        ...
        conn_ppi_db.close()
        tt2=time.time()
        #print ("DAVID",tt2-tt,nominal_ppi_elevation,nobs)
    ...
    print ('pathfileout', pathfileout)

    try:
        combine(pathfileout, filememory)
    except sqlite3.Error as error:
        print(f"Error while creating a single sqlite file:  {os.path.basename(filememory)}", error)
 # close connection 
    conn_filememory.close() 
    tc_total_1 = time.time()
    time_total = tc_total_1-tc_total_0

    print (f'Runtime = ,{round(time_total,2)}, Runtime schema=,{round(time_schema,2)} , Runtime dealias=, {round(time_dealias,2)}')
...
def compute_qcavg(assim_T0,
   ...
   # 
    ...
    if model_outputs_for_dealiasing is not None:
     ...
     #        joblib.delayed(savepkl_for_fulldomain)  (this_model_file,
     #                                                 model_files_list,
     #                                                 model_valid_date_list,
     #                                                 pathoutpickle) for this_model_file in model_files_list)
        ...
        tc_0 = time.time()
      #  joblib.Parallel(n_jobs=100,verbose=100)(
      #       joblib.delayed(savepkl_for_radardomain)(radar_name,
      ...
      #                                         pathoutpickle) for radar_name in vol_scan_dict.keys())
        ...
        delayed_funcs= [dask.delayed(savepkl_for_radardomain)(radar_name,
                                               vol_scan_dict[radar_name]['datetime_list'],
                                               vol_scan_dict[radar_name]['antenna_lat'],
                                               vol_scan_dict[radar_name]['antenna_lon'],
                                               model_files_list,
...
#
       # dask.compute(delayed_funcs)
        tc_1 = time.time()
        print(f"Runtime wind dictionary by radar: {round(tc_1-tc_0,4)} s")
        #exit(0)
    ...
    if n_cpus == 1:
        ...
        for vscan in vol_scan_list[0:10]:
