...
# The node data provider (NDP) data for a model.
#
# It is a dict that indexes NodeDataProviderGraphData by graph ids.
@dataclass
class NodeDataProviderData:
  """The node data provider (NDP) data for a model."""

  # NDP graph data indexed by graph ids.
  graphsData: dict[str, 'NodeDataProviderGraphData']

  def save_to_file(self, path: str, indent=2) -> None:
...
@dataclass
class NodeDataProviderGraphData:
  """NDP data for a single graph."""
  ...
  results: dict[str, 'NodeDataProviderResultData']
...
@dataclass
class NodeDataProviderResultData:
  """A single result data corresponding to a node/output tensor"""
  ...
  # from `NodeDataProviderGraphData.thresholds`
  # `NodeDataProviderGraphData.gradient`.
  #
  # (optional)
