...
class AqGeometricClimatologyModel(BaseModel):
    ...
    def fit(self, g: "Data"):
        ...
        if self.state_dict is None:
            state_dict = {h3_idx: {"means": [], "_count": 0} for h3_idx in graph_h3_index}
        else:
            state_dict = self.state_dict

        for i, h3_idx in enumerate(graph_h3_index):
            if h3_idx not in state_dict:
                state_dict[h3_idx] = {"means": [], "_count": 0}
            state_dict[h3_idx]["means"].append(np.mean(graph_data[i, :, :], axis=0, keepdims=True))
            state_dict[h3_idx]["_count"] += 1
    ...
    def predict(self, g: "Data") -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i, h3_idx in enumerate(h3_indices):
            if h3_idx in self.state_dict:
                    pred[i, :, :] = np.tile(
                        np.mean(self.state_dict[h3_idx]["means"], axis=0),
                        (1, len(target_timestamps), 1)
                    )
            else:
                pred[i, :, :] = np.nan
