...
class ProviderArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[str] api_key: API key for Prowlarr authentication. Can be specified via the `PROWLARR_API_KEY` environment variable.
        :param pulumi.Input[str] authorization: Token for token-based authentication with Prowlarr. This is an alternative to using an API key. Set this via the
               `PROWLARR_AUTHORIZATION` environment variable. One of `authorization` or `api_key` must be provided, but not both.
        :param pulumi.Input[str] url: Full Prowlarr URL with protocol and port (e.g. `https://test.prowlarr.com:9696`). You should **NOT** supply any path
...
class Provider(pulumi.ProviderResource):
    ...
    def _internal_init(__self__,
        ...
        secret_opts = pulumi.ResourceOptions(additional_secret_outputs=["apiKey", "authorization"])
