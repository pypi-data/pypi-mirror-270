...
class BaseJobExecutor(base.Base):
    ...
    def execute(self, data):
        if 'setvar' in data and data['setvar'] in self.var_names:
            ...
            return {'success': False, 'message': 'Unknown Operation (BaseJobExecutor)'}
...
class JobRunner(base.Base):
    ...
    def page_ui(self, params={}):
        p = base.Page(owner=self)
        p.h1("Runner UI")
        p.script(js_functions()+web.js_fn('add', [], [
            'var $ctrls = document.getElementById("ctrls");',
            'var $d = document.createElement("div");',
            '$d.classList.add("entry");'
            '$d.innerHTML = "<input class=\\"name\\" placeholder=\\"Key\\" /><input class=\\"value\\" placeholder=\\"Value\\" />";'
            '$ctrls.appendChild($d);'
            ])+
            web.js_fn('run', [], [
                'var data = {};'
    ...
    def page_registry(self, params={}):
        ...
        for name in parts.names:
            part = parts[name]
            p.div(part.name + " = " + str(part.value) + " " + web.a("Load", '?install=' +part.name))
        p.hr()
        p.a("Runner", '/pysys/runner')
        return p.simple_page()
...
class ProcessExecutor(BaseJobExecutor):
    ...
    def __init__(self, cmd, start=True, restart=False, on_line=None, on_end=None):
        super().__init__()
