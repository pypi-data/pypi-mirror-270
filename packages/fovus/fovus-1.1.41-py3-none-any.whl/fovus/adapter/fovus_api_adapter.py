...
class FovusApiAdapter:
    ...
    def _confirm_enable_preemptible_support(self, request):
        ...
        if (
            ...
            print(
                "Enabling preemptible resources will restrict the maximum task Walltime allowed to 2 hour. Preemptible "
            ...
            if input() == "y":
                ...
                if request[PAYLOAD_CONSTRAINTS][PAYLOAD_TASK_CONSTRAINTS][WALLTIME_HOURS] > 2:
                    print(
                        "Warning: Preemptible resources will restrict the maximum task Walltime allowed to 2 hour. "
                        + "Overriding Walltime from "
                        + f"{request[PAYLOAD_CONSTRAINTS][PAYLOAD_TASK_CONSTRAINTS][WALLTIME_HOURS]} to 2 hour."
                    )
                    request[PAYLOAD_CONSTRAINTS][PAYLOAD_TASK_CONSTRAINTS][WALLTIME_HOURS] = 2
            else:
                request[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS][ALLOW_PREEMPTIBLE] = False
