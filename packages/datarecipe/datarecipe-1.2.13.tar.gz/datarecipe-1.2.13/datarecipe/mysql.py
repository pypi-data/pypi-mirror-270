...
def print_action_result(table, action, df, df_date_col=None):
...
# todo——根据任意列更新数据     
# def custom_update_data(raw_df, yaml_file_name, database, 
#                 table, 
#                 add_data = False, 
...
#                 end_idx = start_idx + chunk_size
#                 df.iloc[start_idx:end_idx].to_sql(table, engine, if_exists='append', index=False)
#             print_action_result(table, "added", df)
#         else:
#             df.iloc[0:1].to_sql(table, engine, if_exists='append', index=False)
#             delete_sql = f"DELETE FROM {table} WHERE {key_col} BETWEEN '{start_date}' AND '{end_date}'"
#             execute_sql(engine, delete_sql)
#             for i in tqdm(range(num_chunks), desc="Updating Data"):
#                 start_idx = i * chunk_size
#                 end_idx = start_idx + chunk_size
#                 df.iloc[start_idx:end_idx].to_sql(table, engine, if_exists='append', index=False)
#             print_action_result(table, "updated", df)
#     except Exception as e:
#         raise ValueError(f"{e}")