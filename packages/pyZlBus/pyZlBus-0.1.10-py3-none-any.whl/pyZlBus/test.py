    ...
    async def listenData(self):
        try:
            ...
            async with BleakClient(self.blueAddr) as self.client:
                ...
                await asyncio.sleep(0.1)
'''
                await self.client.write_gatt_char(CmdCtrl_WriteNotify_UUID, zlb.ul_modifyDataFormat(zlb.e_UpLoadDataForMat.UPLOAD_DATA_TIME.value | 
                                                                                                    zlb.e_UpLoadDataForMat.UPLOAD_DATA_ACC.value |
                                                                                                    zlb.e_UpLoadDataForMat.UPLOAD_DATA_GYRO.value |
                                                                                                    zlb.e_UpLoadDataForMat.UPLOAD_DATA_MAG.value), response=True)
                await asyncio.sleep(0.1)
'''
