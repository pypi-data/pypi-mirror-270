...
class Predibase:
    ...
    def http_post(self, endpoint: str, data: Any | None = None, json: Any | None = None, **kwargs) -> dict:
        ...
        def do():
            return self._http.post(
                self.api_gateway + endpoint, data=data, json=json, headers=self.default_headers, **kwargs
            )
    ...
    def http_put(self, endpoint: str, json: Optional[Any]) -> dict:
        @warn_outdated_sdk
        def do():
    ...
    try:
        ...
        if not isinstance(data, dict):
            ...
            raise PredibaseResponseError(
                f"Invalid response payload - expected a JSON object, got {data}", HTTPStatus.INTERNAL_SERVER_ERROR
            )
