...
class Deployments:
    ...
    def client(self, deployment_ref: str | Deployment) -> LoraxClient:
        if isinstance(deployment_ref, Deployment):
            deployment_ref = deployment_ref.name

        if "/" in deployment_ref:
            raise ValueError(f"Deployment name {deployment_ref} appears to be invalid. Are you providing a Hugging "
            ...
            while True:
                ...
                if resp["type"] == "status":
                    if resp["status"] not in ("ready", "updating"):
                        print(f"Deployment {deployment_ref} is still spinning up. Your prompt may take longer than "
                              f"normal to execute.\n")
