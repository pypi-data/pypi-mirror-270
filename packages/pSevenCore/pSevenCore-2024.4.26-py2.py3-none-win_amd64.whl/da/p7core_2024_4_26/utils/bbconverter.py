...
class _WrappedBlackboxAsProblem(_ProblemGeneric):
  ...
  def evaluate(self, x, mask):
    resp = self._blackbox_ref()._evaluate(x)
...
class _WrappedProblemAsBlackbox(_Blackbox):
  def __init__(self, problem, ignore_objectives=None, custom_bounds=None):
    ...
    if not ignore_objectives:
      self._active_objectives = slice(0, problem().size_f())
    else:
  ...
  def prepare_blackbox(self):
    ...
    for name in self._problem_ref().objectives_names():
  ...
  def evaluate(self, points):
    mask = _numpy.zeros((len(points), self._problem_ref().size_full()), dtype=bool)
    mask[:, self._active_objectives] = 1

    size_x, size_f, size_c = self._problem_ref().size_x(), self._problem_ref().size_f(), self._problem_ref().size_c()
    has_grad, sparse_grad, nonzero_df, nonzero_dx = self._problem_ref().objectives_gradient()
    slice_dfdx = slice((size_f+size_c), (size_f+size_c+len(nonzero_df)) if sparse_grad else (size_f*(1+size_x)+size_c))
    mask[:, slice_dfdx] = True

    resp, mask = self._problem_ref()._evaluate(points, mask, None)
    resp[~mask] = _shared._NONE

    if not self.gradients_enabled:
      return resp[:, :size_f]

    if sparse_grad:
      dfdx = _numpy.zeros((resp.shape[0], size_f*size_x))
      for src, idx_df, idx_dx in zip(resp[:, slice_dfdx].T, nonzero_df, nonzero_dx):
        dfdx[:, idx_df * size_x + idx_dx] = src[:]
      return _numpy.hstack((resp[:, :size_f], dfdx))
    elif size_c:
      for dst, src in zip(resp[:,size_f:size_f*(1+size_f)].T, resp[:, slice_dfdx].T):
        dst[:] = src[:]
...
def _problem_as_blackbox(problem, count, user_catvars, location, ignore_objectives=None):
  ...
  for limit_general, limit_expensive in zip(problem.elements_hint(slice(size_x, None), '@GT/EvaluationLimit'), problem.elements_hint(slice(size_x, None), '@GT/ExpensiveEvaluations')):
  ...
  return _WrappedProblemAsBlackbox(_weakref.ref(problem), ignore_objectives=ignore_objectives, custom_bounds=bounds), bounds, user_catvars, warns, vartypes
...
def _make_blackbox(blackbox, count, user_catvars, location, ignore_objectives=None):
  """
  ...
  if _shared.is_sized(blackbox):
    ...
    return _problem_as_blackbox(blackbox, count, user_catvars, location, ignore_objectives=ignore_objectives)
