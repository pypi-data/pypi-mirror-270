...
def revision_show(service: ResponseModelServiceInfo, number: int, format: str):
    ...
    if format == "yaml":
        ...
        service_form = V1ServeRevisionServiceForm(
            ...
            healthcheck=V1PortPath(path=revision.revision_spec.readiness_probe.path,port=int(revision.revision_spec.readiness_probe.port)) if revision.revision_spec.readiness_probe else None,
            monitoring=[V1PortPath(path=x.path,port=int(x.port)) for x in revision.revision_spec.monitoring] if revision.revision_spec.monitoring else None,
