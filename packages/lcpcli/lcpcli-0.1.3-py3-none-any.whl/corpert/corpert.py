        ...
        if self.mode == "upload":
            ...
            for layer, properties in json_obj.get("layer", {}).items():
                ...
                if (
                    ...
                    and properties.get("contains", "") in (firstClass['token'],firstClass['segment'])
                    ...
                    if properties['contains'] == firstClass['token']:
                        ...
                        aligned_entities_segment[layer.lower()] = {'fn': layerFile, 'properties': properties}
            parser = None
            ...
            for filepath in self._input_files:
                ...
                if os.path.basename(filepath) == "meta.json":
                    continue
                parser = parser or PARSERS[self._determine_format(filepath)](config=json_obj)
                print(filepath)
            ...
            parser.close_upload_files(path = self.output or "./",)
            # Process time-anchored extra layers
            for layer, properties in json_obj.get("layer", {}).items():
                if not properties.get("anchoring",{}).get("time", False) or layer in json_obj["firstClass"].values():
                    continue
                    ...
                    with open(os.path.join(output_path, fn), "w") as output_file:
                        while True:
                            ...
                            if not input_col_names:
                                ...
                                output_cols = [c.strip() for c in input_col_names if c not in ("doc_id","start","end")]
                                assert "doc_id" in input_col_names, IndexError(f"No column named 'doc_id' found in {fn}")
                                assert "start" in input_col_names, IndexError(f"No column named 'start' found in {fn}")
                                assert "end" in input_col_names, IndexError(f"No column named 'end' found in {fn}")
                                ...
                                output_cols = [c.strip() for n, c in enumerate(input_cols) if n not in (doc_id_idx,start_idx,end_idx)]
                                doc_frames = parser.doc_frames[str(input_cols[doc_id_idx])]
                                times = [float(input_cols[x]) for x in (start_idx,end_idx)]
                                start, end = [str(int(times[n]*25.0) + doc_frames[n]) for n in (0,1)]
