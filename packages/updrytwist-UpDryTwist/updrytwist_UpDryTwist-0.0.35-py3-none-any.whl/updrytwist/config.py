...
def strreadNoneOk(object, key: str, default: Optional[str] = None) -> str | None:
    value = dictread(object, key, default)
    return default if value is None else str(value)


def strread(object, key: str, default : Optional[str] = None) -> str:
    value = strreadNoneOk(object, key, default)
...
def floatreadNoneOk(
    ...
    return DictionaryConfigReader(dictionary).floatreadNoneOk(key, default)


def floatread(dictionary, key: str, default: Optional[float] = None) -> float:
    return DictionaryConfigReader(dictionary).floatread(key, default)


def listread(dictionary, key: str, default: Optional[list] = None) -> []:
    return DictionaryConfigReader(dictionary).listread(key, default)

def dictionaryread(dictionary, key: str, default: Optional[list] = None) -> dict[ str, Any ]:
    return DictionaryConfigReader(dictionary).dictionaryread(key, default)
...
class Config:
    ...
    def strValueNoneOk(
        ...
        return strreadNoneOk(self.blockFor(fromBlock), key, default)

    def strValue(
        self, key: str, default: Optional[str] = None, fromBlock: str = None
    ) -> str:
        return strread(self.blockFor(fromBlock), key, default)
    ...
    def floatValueNoneOk(
        ...
        return floatreadNoneOk(self.blockFor(fromBlock), key, default)

    def floatValue(
        self, key: str, default: Optional[float] = None, fromBlock: str = None
    ) -> float:
        return floatread(self.blockFor(fromBlock), key, default)
    ...
    def dictionaryValue(
        ...
        return dictionaryread(self.blockFor(fromBlock), key, default)
    ...
    @staticmethod
    def getInt(key: str, default: Optional[int] = None, fromBlock: str = None) -> int:
    ...
    @staticmethod
    def getDictionary(key: str, default: Optional[dict] = None, fromBlock: str = None) -> dict[ str, Any ]:
        return Config.c.dictionaryValue(key, default, fromBlock)

    def readCommandLine(self):
        parser = gentleargparser.GentlerArgParser(
            description="Generic UpDryTwist Command Parser", conflict_handler="resolve"
        )
...
def loadConfig(optional_config: bool = False):
    try:
        ...
        print(
            "Cannot load configuration from file {}: {}".format(DEFAULT_CONFIG, str(e))
        )
        sys.exit(2)
