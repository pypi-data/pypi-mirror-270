...
class NotifyProxy(BaseServiceProxy):
    ...
    def send_email(self, recipients, subject, body, attachements=None):
    ...
    def send_message(self, channel, message, attachements=None):

        res = super().post(
            "message", params={"channel": channel, "message": message}, headers={"Content-Type": "text/plain"}
        )
        code_to_exception(res)
        return res.doc()
