...
class RedisPool:
    def __init__(self,settings,max_connections:int):
        connection=Connection(host=settings.host,
                             port=settings.port,
                             db=settings.database,
        ...
        self.pool=ConnectionPool(connection_class=connection,max_connections=settings.max_connections)

    def get_connection(self):
        return StrictRedis(connection_pool=self.pool)

    def close(self,connnection:Redis):
