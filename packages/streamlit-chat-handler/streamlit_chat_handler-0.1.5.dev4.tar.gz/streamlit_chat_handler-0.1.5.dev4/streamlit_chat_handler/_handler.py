        ...
        for element_list in element_groups:
            ...
            if last_index and (role == "assistant"):
                *_, cont = st.columns([5, 2])
                ...
                comments_popover.sidebar.slider(
                    "Avaliação",
                    min_value=1,
                    max_value=5,
                    value=5,
                    step=1,
                    key=f"{last_index}_rating",
                )
                
                comments_popover.text_input(
                    label="**Insira seu comentário** *(opcional)*",
                    key=f"{last_index}_comment",
                )

                feedback[last_index] = UserFeedback(
                    rating=st.session_state[f"{last_index}_rating"],
                    comment=st.session_state[f"{last_index}_comment"],
                )
