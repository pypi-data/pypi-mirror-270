...
#disable external server for faster testing and development

XMLConversionSettings.dccRestServer = None
DSITOOLSVERSION = "1.1.0"
timeout=0.5
def createXML(
    dccListDict,
...
def checkDSiToolsVersionCompatibility(versionToCheck: str):
    ...
    """
    # TODO implement useful version checking
    if versionToCheck == DSITOOLSVERSION:
        return True
    else:
        raise RuntimeWarning(
            "Using Data from Different DSI Tool versions. Check for sanity"
        )
        return True
...
class dccQuantityTable:
    def __init__(
        ...
        self.valueVectorsQuantities = []
        self.indexVectorQuantities = [index.quantity for index in self.index]
        ...
        if interpolationTypes != None:
            if len(self.index) > 1:
                ...
                raise RuntimeWarning(
                    "interpolation with multi indexing is not supported"
                )
            else:
                self.interpolators = {}
        for valueQuant in valueQuants:
            self.valueVectorsQuantities.append(valueQuant["quantity"])
            if interpolationTypes != None:
                try:
            ...
            self.__dict__[valueQuant["quantity"]] = valueQuant
        self.language = []
        for vec in indexQuants + valueQuants:
            self.language.append(list(vec.multilangnames.keys()))
        languagesMatch = True
    ...
    def fromDict(cls, dict: dict) -> dccQuantityTable:
        ...
        valueVectorQuantities = dict["valueVectorsQuantities"]
        valueVectors = []
        for valueVectorQuantity in valueVectorQuantities:
            valueVectors.append(dccQuantity.fromJson(dict[valueVectorQuantity]))
        ...
        additionalKeys = dict.keys() - set(
            ["index", "interpolators"] + valueVectorQuantities
        )
        for key in additionalKeys:
            instance.__dict__[key] = dict[key]
        return instance
    ...
    def toDCCquantityList(self):
        ...
        for valueVectorQuant in self.valueVectorsQuantities:
            resultList.append(self[valueVectorQuant].toDCCXMLQuantityStr()["dcc:quantity"])
        return {
            "dcc:list": {
    ...
    def getAllUnits(self):
        ...
        for valueQuant in self.valueVectorsQuantities:
    ...
    def getColsWithUnit(self,unit):
        ...
        if unit is not '':
            ...
            colNames['index'] = self.indexVectorQuantities
            colNames['valueVectors'] = self.valueVectorsQuantities
        return colNames
    ...
    def getOverlappingIDXKeys(self,extIdxVec,indexName=None,tolerance=np.finfo(float).eps):
        ...
        if indexName is not None:
            ownIDX=self.indexVectorQuantities.index(indexName)
    ...
    def __getitem__(self, item):
        ...
        if type(item) == str:
            try:
                ...
                if item in self.indexVectorQuantities:
                    return self.index[self.indexVectorQuantities.index(item)]
                else:
                    raise ke
            ...
            interpolIndexVec = self.index.interpolatedValuesToDSIVector(
    ...
    def __repr__(self):
        ...
        rptStr += (
            " Quantities>"
            + str(self.valueVectorsQuantities)
            + " Interpolation Typ>"
