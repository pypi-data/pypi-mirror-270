...
def test_multiVector():
    ...
    assert testMultVectorMultiIndex.indexVectorQuantities == ['Frequency']
    assert testMultVectorMultiIndex.valueVectorsQuantities == ['Magnitude', 'Phase']
...
def test_multiVectorJSONDumpingAndLoadingFromFile():
    ...
    with open("tests/test_dccQuantTabJSONDumpingAndLoadingFromFile.json",encoding='utf-8') as JSONFile:
        loadedMultiVec=dccQuantityTable.fromJson(JSONFile.read())
