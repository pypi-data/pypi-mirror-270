...
class Search():
    ...
    def call(self, after:int = 0, limit:int = 100) -> requests.Response:
        ...
        if self.full_property:
            properties = read_all.read_all_properties(self.client, Endpoints.PROPERTIES_TICKETS_READ_ALL)["df"]["name"].to_list()
        ...
        response = requests.request("POST", self.client.endpoint(Endpoints.TICKETS_SEARCH), headers=self.client.headers, data=json.dumps(payload))
