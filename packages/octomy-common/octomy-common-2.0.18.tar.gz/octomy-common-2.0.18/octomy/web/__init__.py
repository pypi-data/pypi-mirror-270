...
async def db_get(request, context, query_key, params = dict(), mode="none", item_type = dict | None, prepare=True, do_debug=True):
	...
	if None == db_items and mode != "none":
		raise Exception(db_err or f"Unknown error fetching '{query_key}'")
	return db_items
...
async def db_html(request, context, query_key, item_type = dict | None, template_path = "", template_params = dict(), do_debug=True):
	logger.info(f"db_html(request={request}, context={context}, query_key={query_key}, item_type={item_type})")
	expect_html(request)
	item = await db_get(request = request, context = context, query_key = query_key, params = params, item_type = item_type, prepare=prepare, do_debug = do_debug)
