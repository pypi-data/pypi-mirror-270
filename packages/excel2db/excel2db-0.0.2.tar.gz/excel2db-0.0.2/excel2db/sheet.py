...
class sheet:
    ...
    def sheet(self):
        ...
        self.value.sheetPosition[0], self.value.sheetPosition[2] = self.value.codeToNumber(self.value.sheetConf["position"]) ##获取上行和左列
        ##获取下行
        ...
        if self.value.sheetConf["isIncludeDetail"]: ##若有明细表
            if self.value.sheetConf["detailSplitByColumnID"] != "": ##获取当前分割线
                splitLine = self.value.codeToNumber(self.value.sheetConf["detailSplitByColumnID"])
            else:
                pass
