...
class HasuraClient:
    ...
    def anomalies_training(self, output_dir=None, base64_encoded_data=False, database_output=False,
        ...
        Returns:
            dict: A dictionary containing filenames/path of the saved models. If database_output is True and a MONGODB_CONNECTION_STRING environment variable is set, it return the model
        * key under 'modelKey'.
        ...
        models = {"selectionSetHash": selection_set_hash, "operationName": self.operation_name}
        for key in self.vectorized_result:
            ...
            if base64_encoded_data or database_output:
                models[key] = base64.b64encode(pickle.dumps(clf)).decode('utf-8')
            else:
        ...
        if database_output and os.environ.get("MONGODB_CONNECTION_STRING") is not None:
            ...
            if selection_set_hash is not None:
                collection.update_one({"selectionSetHash": selection_set_hash}, {'$set': models}, upsert=True)
            else:
                collection.update_one({"operationName": self.operation_name}, {'$set': models}, upsert=True)
            client.close()
            return {"modelKey": selection_set_hash}

        return models

    def anomalies(self, training_files=None, threshold=0, training_base64=None, training_database_key=None):
        """
        Args:
            training_files: (optional) A dictionary containing the paths to training files for each key.
            threshold: (optional) The threshold for anomaly detection. Default is 0.
            training_base64: (optional) Base64 encoded serialized training data for each key.
            training_database_key: (optional) The key to fetch training data from a database.

        Returns:
            A dictionary containing the anomalies found for each key.
        ...
        trained_models = {}
        min_features = {}
        if training_database_key is not None:
            connection_string = os.environ.get("MONGODB_CONNECTION_STRING")
            default_db_name = connection_string.split('/')[-1]
            client = MongoClient(connection_string)
            db = client[default_db_name]
            collection = db['anomalyDetectionModels']
            training_base64 = collection.find_one({"hash": training_database_key})
            client.close()
