...
def detect_s3_data_assets(
    ...
    patterns_to_urls = discover_patterns_from_s3_bucket(
        client, strip_s3_bucket_prefix(bucket), include=include
    )
    with ThreadPoolExecutor() as executor:
        results = executor.map(
            lambda entry: (entry[0], get_merged_def_from_s3_files(bucket, *entry)),
...
def get_distinct_dictionaries(dictionaries: list[dict]) -> list[dict]:
    ...
    return sorted(
        [dict(t) for t in {tuple(d.items()) for d in dictionaries}],
        key=lambda x: x["type"],
    )
