...
def collect(app):
    ...
    for method_url, handler in webhook_handlers.items():
        ...
        if len(openapi_route.responses) == 0:
            openapi_route.add_response_content(200, OpenapiRouteContent(description='Ok'))
        if url not in openapi['x-webhooks']:
            openapi['x-webhooks'][url] = {method: openapi_route.to_dict()}
        else:
            openapi['x-webhooks'][url][method] = openapi_route.to_dict()
