...
class Manager:
    ...
    def upload_rec_dir(self,project_name,step_folder,pyom,prefix=""):
        ...
        for subdir in step_subdirs:
            ...
            try:
                ...
                print("Unable to convert images to OMERO format, but continue uploading")
            pyom.upload_omero_set(project_name,dataset_name,os.path.join(step_folder,subdir),include_logs=(os.path.isdir(os.path.join(os.path.join(step_folder,subdir),"LOGS"))))
    ...
    def upload_analysis_dir(self,project_name,step_folder,pyom):
        ...
        for subdir in analysis_subdirs:
            ...
            to_upload = [f for f in os.listdir(dir) if os.path.isfile(os.path.join(dir, f)) and (".mp4" in f or ".png" in f)]
    ...
    def upload_whole_embryo(self,parameters):
        ...
        if project is None:
            ...
            return
        step_subdirs = [f for f in os.listdir(embryo_dir) if os.path.isdir(os.path.join(embryo_dir, f)) and f.lower() in ["fuse","seg","post","background","contour","rec-membrane","rec-seed"]]
