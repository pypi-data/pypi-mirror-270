...
class AstroWeather:
    ...
    async def _get_location_data(self) -> None:
        ...
        # TODO: Make this safe if we don't find a start index or don't have 7timer data
        ...
        item = {
            ...
            "precipitation_amount": (self._weather_data_metno[metno_index].get("data", {}).get("next_1_hours", {}).get("details", {}).get("precipitation_amount", "")),
            # Condition
            "condition_percentage": await self.calc_condition_percentage(
                details_metno.get("cloud_area_fraction", -1),
                seeing,
    ...
    def get_data_seventimer_time(self, anchor_timestamp, datetime):
        seventimer_index = 0
        for row7 in self._weather_data_seventimer:
    ...
    async def _get_forecast_data(self, forecast_type, hours_to_show) -> None:
        ...
        for index, datapoint in enumerate(self._weather_data_metno):
            if forecast_time > datetime.strptime(datapoint.get("time"), "%Y-%m-%dT%H:%M:%SZ"):
                continue
            ...
            row = self.get_data_seventimer_time(seventimer_init, datetime.strptime(datapoint.get("time"), "%Y-%m-%dT%H:%M:%SZ"))
            ...
            seeing = 0
            if row["seeing"] == -1 or row["seeing"] == -9999:
                seeing = await self._astro_seeing.calculate_seeing_model6(
                    temperature=details_metno.get("air_temperature", -1),
                    humidity=details_metno.get("relative_humidity", -1),
                    dew_point_temperature=details_metno.get("dew_point_temperature", -1),
            ...
            item["precipitation_amount"] = self._weather_data_metno[metno_index + cnt].get("data", {}).get("next_1_hours", {}).get("details", {}).get("precipitation_amount", "")
        ...
        #                     dew_point_temperature=details.get("dew_point_temperature", -1),
        #                     wind_speed=details.get("wind_speed", -1),
        #                     cloud_cover=details.get("cloud_area_fraction", -1),
        #                     altitude=self._elevation,
        #                     air_pressure_at_sea_level=details.get("air_pressure_at_sea_level", -1),
        #                 )
        #             else:
        #                 seeing = SEEING[row["seeing"] - 1]
        ...
        for day in range(0, len(start_indexes)):
            ...
            for idx in range(
                ...
                if len(interval_points) <= int(math.floor(night_duration_astronomical / 3600)):
                    interval_points.append(await self.calc_condition_percentage(cloud_area_fraction, seeing, transparency, wind_speed))
    ...
    async def calc_condition_percentage(self, cloudcover, seeing, transparency, wind_speed):
        ...
        condition = int(100 - (self._cloudcover_weight * cloudcover + self._seeing_weight * seeing + self._transparency_weight * transparency + self._calm_weight * wind_speed_value) / (self._cloudcover_weight + self._seeing_weight + self._transparency_weight + self._calm_weight))
    ...
    async def async_request_seventimer(self, product="astro", method="get") -> dict:
        ...
        # STIMER_OUTPUT = "json"
        url = str(f"{BASE_URL_SEVENTIMER}") + "?lon=" + str("%.1f" % round(self._longitude, 2)) + "&lat=" + str("%.1f" % round(self._latitude, 2)) + "&product=" + str(product) + "&output=" + STIMER_OUTPUT
        try:
    ...
    async def async_request_met(self, product="met", method="get") -> dict:
        ...
        url = str(f"{BASE_URL_MET}") + "?lon=" + str("%.1f" % round(self._longitude, 2)) + "&lat=" + str("%.1f" % round(self._latitude, 2)) + "&altitude=" + str(self._elevation)
        try:
            _LOGGER.debug(f"Query url: {url}")
