...
from safefs import console_init_safefs, FileSystem as SafeFileSystem # type: ignore
...
class WrappedFile(object):
    ...
    def x_create_watch(self, cb, completion_filter, watch_tree):
        return self._fs.x_create_watch(cb, self._handle, completion_filter, watch_tree)

    def pread(self, *n):
        return self._fs.pread(self._handle, *n)

    def preadinto(self, *n):
        return self._fs.preadinto(self._handle, *n)
    ...
    def utimes(self, *n, **kw):
        return self._fs.futimes(self._handle, *n, **kw)
...
class SubFileSystem(object):
    def __init__(self, fs, subs):
        ...
        if hasattr(self._subfs, 'x_create_watch'):
            self.x_create_watch = self._x_create_watch

    def futimes(self, handle, *n, **kw):
        return handle.utimes(*n, **kw)
    ...
    def _x_create_watch(self, cb, dir_handle, completion_filter, watch_tree):
        if watch_tree:
            raise NotImplementedError("Can't implement this!")
        return dir_handle.x_create_watch(cb, completion_filter, watch_tree)
    ...
    def replace(self, old_path, new_path):
        ...
        return fs1.replace(old_path, new_path)

    def statvfs(self):
        return self._fs.statvfs()

    def preadinto(self, handle, size, offset):
        return handle.preadinto(size, offset)
