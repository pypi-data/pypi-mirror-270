    ...
    def _fit(self,
        ...
        param_rows = [
            ...
            ('EVALUATION_METRIC', None, None,
             None if self.evaluation_metric is None else \
             (self.evaluation_metric.split('_')[0].upper() + '_' + \
             self.evaluation_metric.split('_')[1].upper() if any(x in self.evaluation_metric.lower() \
             for x in ['f1-score', 'recall', 'precision']) else self.evaluation_metric.upper())),
        ...
        if self.ref_metric is not None:
            param_rows.extend([('REF_METRIC', None, None,
                                ...
                                any(x in metric.lower() for x in ['f1-score', 'recall', 'precision']) \
...
#pylint: disable=too-many-instance-attributes
    ...
    'recall_<class name>', 'precision_<class name>, 'f1-score_<class_name>'}, optional
    ...
    def __init__(self,
        ...
        if self.ref_metric is not None:
            for metric in self.ref_metric:
                ...
                all(x not in metric for x in ['recall', 'precision', 'f1-score']):
