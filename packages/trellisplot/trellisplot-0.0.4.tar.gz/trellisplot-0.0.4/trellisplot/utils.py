...
def jitter(x, y):
    ...
    for unique_val in np.unique(x):
        ...
        if len(y_subset) == 1: # if single value list, don't attempt Gaussian KDE
            ...
            kde_y_subset = 0.375*kde_y_subset/kde_y_subset.max()  # NOTE: this is if i want to normalize for constatn width between trellis
            envelopes= envelopes + list(kde_y_subset)

    envelopes = np.array(envelopes)
    # normalize. if x is filled with zeros, skip normalization step. NOTE: this is if i want to normalize for constant area of violin plots between trellis
    # if envelopes.max() > 0:
