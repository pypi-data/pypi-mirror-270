...
class LassoRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y, **kwargs):
        ...
        try:
            ...
            self.xsd[self.xsd == 0] = 1
            X_ = (X - self.xm[None, :]) / self.xsd[None, :]
            XX = _lassoc.crossprod(X_, backend=self.backend)
            Xy = _lassoc.crossprod(X_, centered_y, backend=self.backend)
            XX2 = 2 * XX
            Xy2 = 2 * Xy
            ...
            invXX = jinv(XX + self.reg_lambda * jnp.eye(X_.shape[1]))
            beta0 = _lassoc.safe_sparse_dot(invXX, Xy, backend=self.backend)
            if len(np.asarray(y).shape) == 1:
                res = _lassoc.get_beta_1D(
                    beta0=np.asarray(beta0),
                    XX2=np.asarray(XX2),
