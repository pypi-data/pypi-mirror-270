...
class RPICommLink:
    ...
    def server_open(self, device_name: str = None, max_connect: int = 0):
        """打开_server线程的函数
        ...
        """
        listen_thread = threading.Thread(target=self._server, args=(device_name, max_connect), daemon=True)
        listen_thread.start()

    def _server(self, device_name, max_connect):
    ...
    def connect_server(self, target_name):
