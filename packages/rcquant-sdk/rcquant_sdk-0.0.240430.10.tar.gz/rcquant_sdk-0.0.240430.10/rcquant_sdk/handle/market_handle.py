...
class MarketHandle():
    ...
    def fin_save_ohlc_list(self, instrument_id: str, period: str, df, **kwargs) -> Tuple[bool, str]:
        ...
        b, m, params = self.__create_fin_persists_save_param_data(instrument_id, df, **kwargs, period=period)
        if b == False:
            return [b, m]
    ...
    def fin_save_tick_list(self, instrument_id, df, **kwargs) -> Tuple[bool, str]:
        ...
        b, m, params = self.__create_fin_persists_save_param_data(instrument_id, df, **kwargs)
        if b == False:
            return [b, m]
    ...
    def __create_fin_persists_save_param_data(self, instrument_id: str, df, **kwargs):
        if not isinstance(df, pd.DataFrame):
            return [False, "df 数据类型格式不是 DataFrame", {}]
