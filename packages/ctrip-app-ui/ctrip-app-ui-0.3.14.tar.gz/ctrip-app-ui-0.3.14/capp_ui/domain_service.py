...
class CtripAppService(PlatformService):
    ...
    def touch_my(self) -> None:
        """进入app后，点击【我的】"""
        try:
            my_button_1 = self.device.poco(type="android.view.ViewGroup", name="ctrip.android.view:id/a", desc="我的")
            my_button_2 = self.device.poco(
                type="android.widget.TextView", name="ctrip.android.view:id/home_tab_bar_text_tv", desc="我的"
            )
            if my_button_1.exists() is True:
                my_button_1.click()
            elif my_button_2.exists() is True:
                my_button_2.click()
            else:
                logger.warning("没有定位到【我的】首页标签")
