import os
from tdf_tool.tools.translate.ios.tools.batch_pod_tools import (
    BatchPodTools,
)
from tdf_tool.tools.cmd import Cmd
from tdf_tool.tools.fix_header.fix_header_lint_tool import FixHeaderLintTool
from tdf_tool.tools.fix_header.fix_header_replace_tool import FixHeaderReplaceTool
...
class FixHeaderEntry:
    ...
    def module(self, moduleName: str):
        ...
        if moduleName in batchPodNames:
            targetModule = list(
                filter(lambda x: x.name == moduleName, batchPodList))[0]
            if self.fix_type == FixHeaderType.LINT:
