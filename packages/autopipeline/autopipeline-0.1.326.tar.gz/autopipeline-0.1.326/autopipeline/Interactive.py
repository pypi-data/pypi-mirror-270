...
def leap(query, table_ptr, description, verbose = False, udf = None, saving_mode=False):
    ...
    hint = ls[1]
    if res == "False":
        print("########## Feedback: ", hint + " Please Be More Detailed on Query.")
    ...
    if require_new:
        print("########## Feedback: ", feedback + " Please Be More Detailed on Query.")
    else:
        print("Succeed!")
    return result, table

def leap_demo(query, table_ptr, description, verbose = False, udf = None, saving_mode=False):
