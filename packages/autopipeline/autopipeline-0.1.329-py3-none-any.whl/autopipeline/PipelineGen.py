...
def pipeline_gen(query, table, function_chain, desc, verbose, client, udf, gpt4):
    ...
    while 'displayed' not in status:
        ...
        if verbose:
            print("Current completed stages: ", status)
            print("Current table descriptions: ", desc)
...
def pipeline_gpt(user_query, column_description, description, status, verbose, client, gpt4):
    ...
    if gpt4:
        ...
        if verbose:
            ...
            print("Number of tokens of messages for 'pipeline_gpt': ", num_token_msg)
            print("Number of tokens of functions for 'pipeline_gpt': ", num_token_func)
            print("Number of prompt tokens for 'pipeline_gpt': ", response.usage.prompt_tokens)
            print("Number of answer tokens for 'pipeline_gpt': ", response.usage.completion_tokens)
            print("Number of total tokens for 'pipeline_gpt': ", response.usage.total_tokens)
            current_pize = 0.00003 * response.usage.prompt_tokens + 0.00006 * response.usage.completion_tokens
            print("Cost for 'pipeline_gpt': $", current_pize)
            autopipeline.cost += current_pize
            print("Accumulated cost: $", autopipeline.cost)
