...
def query_gen(user_query, enum, desc, status, verbose, client, gpt4):
    ...
    if gpt4:
        ...
        if verbose:
            ...
            print("Number of prompt tokens for 'query_gen': ", response.usage.prompt_tokens)
            print("Number of answer tokens for 'query_gen': ", response.usage.completion_tokens)
            print("Number of total tokens for 'query_gen': ", response.usage.total_tokens)
            current_pize = 0.00003 * response.usage.prompt_tokens + 0.00006 * response.usage.completion_tokens
            print("Cost for 'query_gen': $", current_pize)
