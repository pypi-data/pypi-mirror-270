...
#Document-------------------------------------------------------------------------------------------------------------------------------------------
    ...
    class DocumentExistsRequest(BaseModel):
        file_path: str
        collection_name: str
    async def document_exists_async(self, collection_name, file_path):
        try:
            url = os.path.join(self.base_url,f"collection/{collection_name}/document/exists")
            ...
            logger.error(f"RAGClientAsync.document_exists_async: Error={e}")
            raise e
...
#Chunks-------------------------------------------------------------------------------------------------------------------------------------------
    ...
    async def list_chunks_by_chunkgroup_id_async(self,chunkgroup_id):
        url = os.path.join(self.base_url,f"chunks/{chunkgroup_id}")
        response = await http_get_async(url)
        return json.loads(response.text)
    
    async def get_chunk_async(self,chunk_id):
        url = os.path.join(self.base_url,f"chunk/{chunk_id}")
        response = await http_get_async(url)
        return json.loads(response.text)