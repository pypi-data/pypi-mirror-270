...
class Settings:
    ...
    #     super().__init__()
    #     # 将所有传入的关键字参数作为属性添加到实例中
    #     for key, value in kwargs.items():
    #         setattr(self, key, value)

    # return {field.name: field for field in self.__attrs_attrs__}

    Extra = attrib(type=dict, default=dict())
    Timeout = attrib(type=float, default=os.environ.get("Timeout", 2), converter=float)

    # redis 表名
    # 任务表模版
    TAB_REQUESTS = attrib(type=str, default="{redis_key}:z_requests")
    # 任务失败模板
    TAB_FAILED_REQUESTS = attrib(type=str, default="{redis_key}:z_failed_requests")
    # 数据保存失败模板
