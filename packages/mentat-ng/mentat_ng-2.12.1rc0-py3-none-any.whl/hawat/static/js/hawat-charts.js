...
function table_value_formatter(formatter) {
    ...
    return function(value) {
        try {
            if (value_formatter.format !== undefined) {
...
function byteFormatter() {
    let value_formatter = Intl.NumberFormat(GLOBAL_LOCALE, {
    ...
    return function(size) {
        ...
        return `${value_formatter.format(size)} ${units[i]}`;
    }
}
...
function render_table_timeline_multi(tid, table_columns, table_data, data_stats) {
    ...
    var value_formatter = table_value_formatter(Intl.NumberFormat(GLOBAL_LOCALE));
    var date_formatter = table_value_formatter(Intl.DateTimeFormat(GLOBAL_LOCALE));
	...
	table.attr('class', 'table table-bordered table-striped table-sm table-dataset table-dataset-timeline')
...
function render_action_dropdown(actions, args, kwargs) {
    ...
    div_slct = d3.select(root_elem)
        .attr('class', 'd-inline-block ms-2');

    div_dropdown = div_slct.append('div')
        .attr('class', 'btn-group');

    div_dropdown.append('button')
        .attr('class', 'btn btn-outline-dark btn-xs btn-sm dropdown-toggle ms-2')
        .attr('type', 'button')
...
function render_table_dict(tid, table_columns, table_data, data_stats, action_list = null, params = {}) {
    ...
    var number_formatter = table_value_formatter(
        Intl.NumberFormat(GLOBAL_LOCALE)
    );
    ...
    table.attr('class', 'table table-bordered table-striped table-sm table-dataset table-dataset-pie')
    ...
    if (!params.hide_table_colors) {
        brows.select('td')
            ...
            .attr('class', 'fas fa-fw fa-circle float-start');
    }
