...
const CACHE_NAME = "sketchbook-cache-202404301";
...
const PYSCRIPT_CONFIG = {
    "packages": ["/dist/sketchingpy-0.2.1-py3-none-any.whl"]
};
...
async function interceptRequest(request) {
    ...
    if (currentHost !== url.hostname) {
        ...
        const filenamesFuture = navigator.storage.getDirectory()
            .then((directory) => Array.fromAsync(directory.keys()));
