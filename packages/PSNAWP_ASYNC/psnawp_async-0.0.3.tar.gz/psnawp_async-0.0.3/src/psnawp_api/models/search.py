...
class Search:
    ...
    async def universal_search(self, search_query: str, limit: int = 20) -> dict[str, Any]:
        ...
        response: dict[str, Any] = await self._request_builder.post(url=f"{BASE_PATH['universal_search']}", data=json.dumps(data))
        json1 = response.json()
        return json1
    ...
    async def get_title_id(self, title_name: str) -> tuple[str, str]:
        ...
        response = await self._request_builder.post(url=f"{BASE_PATH['universal_search']}", data=json.dumps(data))
        json1 = response.json()
        result = json1["domainResponses"][0]["results"]
        if len(result) >= 1:
            t: tuple[str, str] = (
