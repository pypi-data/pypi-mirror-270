...
class DatabaseQueue(OurQueue, GenericCRUD, metaclass=MetaLogger, object=queue_logger_object):
    ...
    def push(self, data_json: dict) -> int:
        ...
        if self.id_column_name in data_json:
            id_column_value = data_json[self.id_column_name]
            del data_json[self.id_column_name]
            self.update_by_id(id_column_value=id_column_value, data_json=data_json)
            # TODO push_result = id_column_value
            return id_column_value
        else:
            # TODO push_result = elf.insert(data_json=data_json)
            return self.insert(data_json=data_json)
        # TODO Can we do this everywhere?
        # TODO logger.end("...", {'push_result': push_result} )
        # TODO return push_result
    ...
    def peek(self, action_ids: tuple = ()) -> dict:
        ...
        action_ids = self._fix_action_ids(action_ids)
        where = f"process_id IS NULL AND (queue_status_id IS NULL OR queue_status_id = {QueueStatusEnum.NEW.value})" + \
                (f" AND action_id IN {action_ids}" if action_ids else "")
