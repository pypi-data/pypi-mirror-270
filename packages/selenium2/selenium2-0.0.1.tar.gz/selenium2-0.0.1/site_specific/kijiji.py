...
class Kijiji(SiteBehaviour):
    ...
    def __init__(self, browser):
        ...
        self.log = Logger.get_logger('kijiji')
    ...
    def create_content(self, details: dict) -> str:
        ...
        limit_message = self.browser.find_element(
        ...
        for attempt in range(4):
            try:
                ...
                sleep(1)
        return self.browser.wait_for_element('//a[starts-with(@class,"adId-")]').text
    ...
    def is_signed_out(self) -> bool:
        ...
        """
        try:
            self.browser.wait_for_element("@Sign In", timeout=40)
    ...
    def sign_in(self, details: dict, cookies: str = None) -> NoReturn:
        ...
        if cookies is None:
            ...
            self.browser.goto(self.sign_in_page)
            if self.is_signed_out():
                self.log.info("Signing in to kijiji without loading cookies.")
                self.browser.send_keys('#emailOrNickname', username)
                self.browser.send_keys('//html', Keys.TAB)  # send TAB to <html> tag
                self.browser.send_keys('#password', password)
                self.browser.find_element("//label[starts-with(@class,'checkboxLabel')]").click()
            ...
            self.browser.goto(self.homepage)
            self.log.info("Signing in to kijiji by loading cookies.")
            try:
                self.browser.load_cookies(cookies)
            except RuntimeError as e:
                self.log.info(e)
                self.sign_in(details)
                self.browser.save_cookies(cookies)
                return
            self.browser.goto(self.homepage)
            if self.is_signed_in():
    ...
    def sign_out(self) -> NoReturn:
        ...
        if 'www.kijiji.ca' not in url:
            self.browser.goto(self.my_settings)
        if self.is_signed_in():
            self._dismiss_modal()  # dismiss the 'enable notifications?' modal if it pops up
            dropdown = self.browser.wait_for_element(
                "//button[contains(@class,'control')]")
            dropdown.click()
            logout = self.browser.wait_for_element(
    ...
    def delete_ad_with_id(self, ad_id: Any) -> bool:
        ...
        self.browser.goto(self.my_ads)  # (1)
        try:
            self._sort_my_ads() # (2)
            self.browser.wait_for_element(
            ...
            return False
        ads = self.browser.find_elements(
        ...
        for ad in ads: # (4)
            if str(ad_id) in ad.get_attribute('data-qa-id'):
                ...
                self.browser.wait_for_element(
                    '//tr[starts-with(@class,"item")]')
                sub_ads = self.browser.find_elements(
                    '//tr[starts-with(@class,"item")]')
    ...
    def delete_all_ads(self, previous: int = -1) -> bool:
        ...
        if not self.is_signed_in():
            raise RuntimeError('Attempted to delete all ads, but the user is '
                               'not signed in.')
        self.browser.goto(self.my_ads) #(1)
        self.browser.wait_for_element('//tr[starts-with(@class,"item")]')
        ads = self.browser.find_elements('//tr[starts-with(@class,"item")]') # (2)
        ...
        for ad in ads:
            ...
            self.browser.wait_for_element('.//button[starts-with(@class,"actionLink-")]',
                                          parent=ad).click() #(3)
            self.browser.wait_for_element(
                "//button[text()='Prefer not to say']"
            ).click()
            self.browser.find_element("//button[text()='Delete My Ad']").click()
            self.browser.wait_for_element("//button[@id='modalCloseButton']").click()
            sleep(2)
        self.browser.goto(self.my_ads)  #(4)
        try:
            return self.browser.wait_for_page_to_contain('have no active ads')
        except TimeoutException:
    ...
    def delete_oldest_ad_of_category(self, user_bread_crumbs:str) -> bool:
        ...
        try:
            ...
            return False
        ads = self.browser.find_elements('//tr[starts-with(@class,"item")]') #(3)
        urls:List[Tuple] = [] #('href','adId')
        for ad in ads: # (4)
            href = self.browser.wait_for_element(
                parent=ad,
    ...
    def get_active_posts_ids(self) -> List[str]:
        ...
        if not self.is_signed_in():
            raise RuntimeError('Attempted to count active posts, but the user is '
                               'not signed in.')
        self.browser.goto(self.my_ads)
        ads = self.browser.find_elements('//tr[starts-with(@class,"item")]')
        ids = []
        for ad in ads:
            raw_id = ad.get_attribute('data-qa-id')
    ...
    def log_alert_message(self) -> NoReturn:
        ...
        alert_banner = self.browser.find_element(
    ...
    def _dismiss_obstacles(self):
        ...
        term_confirmation = self.browser.find_element(
            "#PostAdConfirmationOfTerms",
            required=False
        )
        if term_confirmation is not None:
            self.browser.click_element("//label[@for='PostAdConfirmationOfTerms']")

        plan_selection = self.browser.find_element(
            "//button[starts-with(@data-qa-id,'package-0-bottom')]",
            required=False
        )
        if plan_selection is not None:
            self.browser.click_element(plan_selection)
    ...
    def _go_to_post_page(self, category_id: U[str, int]) -> NoReturn:
        ...
        :return: NoReturn
        """
        self.browser.log.info('Switching to post page of category ID: "%s"' % category_id)
        self.browser.goto(self.select_category.format(category_id))
    ...
    def _assert_post_success(self) -> NoReturn:
        ...
        # self.browser.wait_for_page_to_contain('You have successfully posted your ad') # slow to respond
        url = self.browser.get_url()
        for e in range(6):
    ...
    def _select_location(self, location_id:U[str, int])->NoReturn:
        ...
        :return: NoReturn
        """
        self.browser.log.info('Selecting user location: "%s"' % location_id)
        self.browser.goto(self.select_location.format(location_id))
    ...
    def _send_parameter(self, parameter: dict) -> NoReturn:
        ...
        if parameter['name'] == 'location':
            for attempt in range(8):
                ...
                change_loc = self.browser.find_element(
                ...
                if change_loc is not None:
                    ...
                    self.browser.scroll_element_into_view(change_loc)
                    change_loc.click()
                ...
                for selector in selectors:
                    field = self.browser.find_element(selector, required=False)
                    if field is not None:
                        field.clear()
                        sleep(0.25)
                        self.browser.send_keys(field, parameter['value'])
                        try:
                            self.browser.wait_for_element(
                            ...
                            cancel = self.browser.find_element('//button[@id="cancelButton"]',
                                                               required=False)
        ...
        self.browser.scroll_element_into_view(field)
        sleep(0.5)
        if field_tag == 'textarea':  # Textarea
            ...
            if field_type == 'text':
                ...
                label = self.browser.find_element("//label[@for='%s']" % field_id)
                self.browser.set_focus_to_element(label)
                # self.browser.scroll_element_into_view(label)
                sleep(0.5)
                self.browser.find_element("//label[@for='%s']" % field_id).click()
                sleep(2)
        elif field_tag == 'select':  # Select
            self.browser.select_from_list_by_value(field, parameter['value'])
        else:
    ...
    def _find_input_field(self, paramater):
        ...
        for selector in selectors:
            fields = self.browser.find_elements(selector, required=False)
            for field in fields:
    ...
    def _sort_my_ads(self):
        ...
        class_name = self.browser.wait_for_element(
        ...
        while 'isSelected' not in class_name or 'ascending' not in class_name:
            ...
            class_name = self.browser.find_element(
                "//th[starts-with(@class,'expiresHeader')]//button/*", # SVG element
            ).get_attribute('class')
            sleep(1)
    ...
    def _submit_post(self, attempts: int=1)->NoReturn:
        ...
        if attempts <= 8:
            try:
                btn = self.browser.find_element("//form[starts-with(@id,'PostAd')]")
    ...
    def _upload_images(self, path: str) -> NoReturn:
        ...
        :return: NoReturn
        """
        self.browser.log.info('Uploading images.')
        files = [os.path.join(path, f) for f in os.listdir(path)
                 if (os.path.isfile(os.path.join(path, f)) and f != '.gitkeep')]
        if len(files) > 0:
            files_string = '\n'.join(files)
            upload_inp = self.browser.find_element('//input[starts-with(@id,"html5")]')
            upload_inp.send_keys(files_string)
            iterations = 1
            while self.browser.find_element('//li[@class="uploading thumbnail"]',
                                            required=False) is None:
                self.browser.log.info('Waiting for `images loading` icon.')
                sleep(0.5)
                iterations = iterations + 1
                if iterations > 20:  # wait up 10 seconds
                    self.browser.log.info('Failed to find `images loading` icon.')
                    break
            self.browser.wait_for_element('//li[@class="uploading thumbnail"]', negate=True,
                                          timeout=120)
            self.log_alert_message()
        else:
            self.browser.log.info('No images to upload.')
    ...
    def _get_bread_crumbs(self):
        ...
        for e in range(seconds):
            for locator in locators:
                bread_crumbs = self.browser.find_element(
                    locator,
                    required=False
                )
