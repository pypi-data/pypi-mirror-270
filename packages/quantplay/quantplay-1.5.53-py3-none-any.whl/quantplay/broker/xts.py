...
from quantplay.exception.exceptions import TokenException, BrokerException
import requests
...
class XTS(Broker):
    ...
    def positions(self):
        ...
        try:
            ...
            Constants.logger.error(f"[XTS_ERROR] Bad api response {api_response}")
            raise BrokerException("Unable to fetch positions from XTS server")
