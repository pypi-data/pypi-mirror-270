...
app = Flask(__name__)
assets = FlaskAssetsPipeline(app, bundles=[
    'app.js',
    'app.css'
])
```
...
```python
assets = FlaskAssetsPipeline(app, bundles=['app.js'], assets_folder='assets')
```
...
```python
assets = FlaskAssetsPipeline(app, ..., inline=True)
```
...
```python
assets = FlaskAssetsPipeline(app, include=['app.js', 'app.css'])
```
...
>
> ```
> assets = FlaskAssetsPipeline(app, include=['import app.js', 'app.css'])
> ```
...
```python
assets = FlaskAssetsPipeline(app, ..., import_map={"mypkg": "https://..."})
...
```python
assets = FlaskAssetsPipeline(app, ..., expose_node_packages=["mypkg"])
...
```python
assets = FlaskAssetsPipeline(app, include=["preload dependency.js", "preload as image cat.jpg", "prefetch page.js"])
```
...
```python
assets = FlaskAssetsPipeline(app, include=["http://example.com/external.js#integrity=sha384-XXXX"])
```
...
```python
assets = FlaskAssetsPipeline(app, ..., cdn_host="https://xxx.cloudfront.net")
```
