...
def dms_path(date):
    ...
    if date < datetime(2023,7,4).replace(tzinfo=pendulum.timezone("UTC")): 
        ...
        logging.info(f"Set bucket: {bucket}")
    elif (date >= datetime(2024,4,15).replace(tzinfo=pendulum.timezone("UTC"))) and (date < datetime(2024,4,25).replace(tzinfo=pendulum.timezone("UTC"))):
        bucket = "klog-co-prod-20240415"
        logging.info(f"Set bucket: {bucket}")
    elif (date >= datetime(2024,4,26).replace(tzinfo=pendulum.timezone("UTC"))):
