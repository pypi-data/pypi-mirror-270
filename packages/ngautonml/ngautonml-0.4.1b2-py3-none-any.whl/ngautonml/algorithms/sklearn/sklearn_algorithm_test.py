...
# pylint: disable=missing-function-docstring, disable=super-init-not-called
...
@pytest.mark.parametrize('name,value,precision', [
    ...
    ('sklearn.ensemble.AdaBoostClassifier', 0.88, 1e-6),
...
def test_probability_output():
    ...
    assert isinstance(got.probabilities, pd.DataFrame)
    assert got.probabilities.shape == (3, 1)
    assert got.probabilities.iat[0, 0] == 'predict_proba'
