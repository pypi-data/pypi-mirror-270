...
class SimpleImputerModelInstance(FittableAlgorithmInstance):
    ...
    def _split_df_by_col_type(cls,
        ...
        numeric_df = data_df.select_dtypes(include=['floating'])
        integral_df = data_df.select_dtypes(include=['integer'])
        categorical_df = data_df.select_dtypes(include=['object', 'bool'])
        other_df = data_df.select_dtypes(exclude=['floating', 'object', 'bool', 'integer'])
