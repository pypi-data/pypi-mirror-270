...
class MemoryDataLoader(DataframeLoader):
    ...
    def __init__(self, config: DatasetConfig):
        if DatasetConfig.Keys.TRAIN_DATA.value not in config.params:
            # TODO(Merritt/Piggy): add test for this error state
            raise MissingKeyError(
                'Attempt to create a memory dataloader, but train_data is not provided.'
            )

        train_df_var_name = config.params[DatasetConfig.Keys.TRAIN_DATA.value]
        ...
        self._test_df = None
        if DatasetConfig.Keys.TEST_DATA.value in config.params:
            test_df_var_name = config.params[DatasetConfig.Keys.TEST_DATA.value]
