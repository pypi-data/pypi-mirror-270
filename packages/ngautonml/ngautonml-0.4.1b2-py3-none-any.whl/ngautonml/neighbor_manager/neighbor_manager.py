...
class NeighborManager(metaclass=abc.ABCMeta):
    ...
    def send(self, node: NodeID, payload: bytes) -> None:
        '''Send a paylod to a specific neighbor, with logging.'''
        mlflow.log_metric(key='PayloadSize', value=len(payload))
        self._send(node=node, payload=payload)

    @abc.abstractmethod
    def _send(self, node: NodeID, payload: bytes) -> None:
        '''Send a payload to a specific neighbor.'''
