...
class DatasetConfig(ConfigComponent):
        ...
        if self._exists(self.Keys.PARAMS):
            ...
            if not isinstance(params_clause, Dict):
                errors.append(InvalidValueError(
                    f'Expected dict at {ProblemDefKeys.DATASET.value}'
    ...
    def dataloader_tags(self) -> Dict[str, str]:
        ...
        return {
            self.Keys.INPUT_FORMAT.value: input_format,
            self.Keys.LOADED_FORMAT.value: loaded_format,
        }
    ...
    def params(self) -> Dict[str, Any]:
        ...
        if self._exists(self.Keys.TRAIN_PATH):
            default_params[self.Keys.TRAIN_PATH.value] = self._get(self.Keys.TRAIN_PATH)
        if self._exists(self.Keys.TEST_PATH):
            default_params[self.Keys.TEST_PATH.value] = self._get(self.Keys.TEST_PATH)
        if self._exists(self.Keys.STATIC_EXOGENOUS_PATH):
            default_params[self.Keys.STATIC_EXOGENOUS_PATH.value] = (
                self._get(self.Keys.STATIC_EXOGENOUS_PATH))
        if self._exists(self.Keys.TRAIN_DIR):
            default_params[self.Keys.TRAIN_DIR.value] = self._get(self.Keys.TRAIN_DIR)
        if self._exists(self.Keys.TEST_DIR):
            default_params[self.Keys.TEST_DIR.value] = self._get(self.Keys.TEST_DIR)
