...
class MemoryCatalog(Catalog[T]):
    ...
    def load(self, module_directory: Path):
        ...
        for rootdir, dirs, filenames in os.walk(module_directory, topdown=True):
            if CATALOG_IGNORE in dirs:
                logging.info('dropping %s/%s', rootdir, CATALOG_IGNORE)
    ...
    def _skip(self, rootdir, filename) -> bool:
        ...
        if not filename.endswith('.py'):
            logging.info('autoloader dropping non-python file %s/%s', rootdir, filename)
            return True
        if filename.startswith('__'):
            logging.info('autoloader dropping special file %s/%s', rootdir, filename)
            return True
        if filename.endswith('_test.py'):
            logging.info('autoloader dropping test path %s/%s', rootdir, filename)
            return True
        return False
    ...
    def _load_module(self, filepath, modname):
        ...
        if modname in sys.modules:
            ...
            logging.info('%s is already loaded', modname)
