...
async def candlestick_json_api(ticker: str, start_date: str, end_date: str):
    try:
        # Call the candlestick_json function and return its result
        result = candlestick_json(ticker, start_date, end_date)
...
@app.get("/correlation_json/{ticker}/{indicator}/{country}/{start_date}/{end_date}")
async def correlation_json_api(ticker: str, indicator: str, country: str, start_date: str, end_date: str):
    try:
        # Call the candlestick_json function and return its result
        result = correlation_json(ticker, indicator, country, start_date, end_date)
        if result.get("message"):
            raise HTTPException(status_code=404, detail=result["message"])
        return result
    except Exception as e:
...
@app.get("/bollinger_bands_json/{ticker}/{start_date}/{end_date}")
async def bollinger_bands_json_api(ticker: str, start_date: str, end_date: str):
    try:
        # Call the candlestick_json function and return its result
        result = bollinger_bands_json(ticker, start_date, end_date)
        if result.get("message"):
            raise HTTPException(status_code=404, detail=result["message"])
        return result
    except Exception as e:
...
@app.get("/indicator_for_countries_json/{indicator}/{countries}/{start_date}/{end_date}")
async def indicator_for_countries_json_api(indicator: str, countries: str, start_date: str, end_date: str):
    try:
        # Convert countries from string to list
        countries = countries.split(',')
        
        # Call the candlestick_json function and return its result
        result = indicator_for_countries_json(indicator, countries, start_date, end_date)
...
@app.get("/stock_correlation_json/{tickers}/{start_date}/{end_date}")
async def stock_correlation_json_api(tickers: str, start_date: str, end_date: str):
    try:
        # Convert tickers from string to list
        tickers = tickers.split(',')
        
        # Call the candlestick_json function and return its result
        result = stock_correlation_json(tickers, start_date, end_date)
...
@app.get("/stock_prediction_json/{ticker}/{start_date}/{end_date}")
async def stock_prediciton_json_api(ticker: str, start_date: str, end_date: str):
    try:
        
        # Call the candlestick_json function and return its result
        result = stock_prediction_json(ticker, start_date, end_date)
...
@app.get('/portfolio_return_json/{tickers}/{start_date}/{end_date}')
async def portfolio_return_json_api(tickers: str, start_date: str, end_date: str):
    try:
        # Convert tickers from string to list
        tickers = tickers.split(',')

        # Call the function and get the base64 encoded image
        image_base64 = portfolio_return_json(tickers, start_date, end_date)
