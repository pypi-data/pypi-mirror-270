...
def temporary_file(suffix='', dir=None):
    dir = dir or os.getcwd()  # Use the current directory if no directory is provided
    temp_filename = os.path.join(dir, f"{uuid.uuid4()}{suffix}")
    try:
        with open(temp_filename, 'w'):  # Ensure the file is created
            pass
        yield temp_filename
    finally:
        try:
            ...
            pass  # If the file does not exist, that's okay

# Candlestick chart with JSON response
def candlestick_json(ticker, start_date, end_date):
    try:
        ...
        if stock_data.empty:
            ...
            return {"message": f"No data available for {ticker} from {start_date} to {end_date}"}
...
# Correlation JSON between stock and economic data
def correlation_json(ticker, indicator, country, start_date, end_date):
    """
...
# Bollinger Bands with JSON response
def bollinger_bands_json(ticker, start_date, end_date):
    try:
        # Fetch the stock data
...
# Indicator for countries with JSON response
def indicator_for_countries_json(indicator, countries, start_date, end_date):
    try:
        fig, ax = plt.subplots(figsize=(10,6))
...
# Stock correlation JSON
def stock_correlation_json(tickers, start_date, end_date):
    try:
        # Get the stock data
...
def stock_prediction_json(ticker, start_date, end_date):
    try:
        # Suppress warnings
...
# Portfolio return with JSON response
def portfolio_return_json(tickers, start_date, end_date):
    """
    ...
    Returns:
    dict: Dictionary containing the image URL of the plot.
    """
    try:
